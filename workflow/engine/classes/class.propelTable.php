...
class propelTable
{
    ...
    // These are not considered to build the sql queries (update,insert,delete)
    public $notFields='  title button linknew begingrid2 endgrid2 ';
    ...
    //This attribute is used to set STYLES to groups of TD, using the field type "cellMark"
    //(see XmlForm_Field_cellMark)
    public $tdStyle='';
    public $tdClass='';
    ...
    function renderTitle()
    {
        ...
        foreach ($this->fields as $r => $rval) {
            if (($this->style[$r]['showInTable'] != '0')
                &&(!(in_array($this->fields[$r]['Name'],$this->masterdetail)))) {
                //if (($this->style[$r]['showInTable'] != '0'))
                $this->tpl->newBlock("headers");
                $sortOrder = (((isset($this->aOrder[$this->fields[$r]['Name']]))
                    && ($this->aOrder[$this->fields[$r]['Name']]==='ASC'))?'DESC':'ASC');
                $sortOrder = (((isset($this->aOrder[$this->fields[$r]['Name']]))
                    && ($this->aOrder[$this->fields[$r]['Name']]==='DESC'))?'':$sortOrder);
                $this->style[$r]['href'] = $this->ownerPage . '?order=' .
                    ($sortOrder !=='' ? (G::createUID('',$this->fields[$r]['Name']) . '=' . $sortOrder):'')
                    . '&page=' . $this->currentPage;
                $this->style[$r]['onsort'] = $this->id . '.doSort("'.G::createUID('',$this->fields[$r]['Name']).
                ...
                */
                ...
                if ($this->style[$r]['titleVisibility'] != '0') {
                    $sortOrder = (((isset($this->aOrder[$this->fields[$r]['Name']]))
                        && ($this->aOrder[$this->fields[$r]['Name']]==='ASC'))?'<img src="/images/arrow-up.gif">':'');
                    $sortOrder = (((isset($this->aOrder[$this->fields[$r]['Name']]))
                        && ($this->aOrder[$this->fields[$r]['Name']]==='DESC'))?
                        '<img src="/images/arrow-down.gif">' : $sortOrder);
                    $this->tpl->assign("header" , $this->fields[$r]['Label'] . $sortOrder);
                    $this->tpl->assign('displaySeparator',
                    (($this->colCount==0)||(!isset($this->fields[$r]['Label']))||($this->fields[$r]['Label']===''))?
                    'display:none;' : '');
                } else {
    ...
    function renderField($row, $r, $result)
    {
        ...
        $fieldClassName = isset($styleData['colClassName']) && ($styleData['colClassName']) ?
            $styleData['colClassName'] : $this->tdClass;
        if ($fielDataName != '') {
        ...
        $alignAttr = (isset($this->style[$r]['align']) && strlen($this->style[$r]['align']>0)) ?
            " align=\"" . $this->style[$r]['align'] . "\"" : '';
        ...
        if (array_search('renderTable', get_class_methods($fieldClass))!== false) {
            ...
            if ($testValue>0 && (isset($value[1]) && strlen(trim($value[1])) == 0)) {
                ...
                // $this->tpl->assign("value" , (preg_match('^[[:space:]]^', $value)
                //&& (substr($fieldName,0,3)!="PRO"))? str_ireplace(" ","&nbsp;",$htmlField):$htmlField);
            } else {
                $this->tpl->assign("value" , $htmlField);
            }
            /* $testValue = preg_match("/<\/?\w+((\s+(\w|\w[\w-]*\w)(\s*=\s*(?:\".*?\"|'.*?'|[^'\">\s]+))?)
            //+\s*|\s*)\/?>/i",$value, $matches);  */
            //      if (empty($matches)){
            //        $this->tpl->assign("value" , (preg_match('^[[:space:]]^', $value) && (substr($fieldName,0,3)
            //          !="PRO"))? str_ireplace(" ","&nbsp;",$htmlField):$htmlField);
            //      } else {
            //        $this->tpl->assign("value" , $htmlField);
            //      }
        }
    ...
    function defaultStyle()
    {
        foreach ($this->fields as $r => $rval) {
            ...
            //Data source:
            if (!(strpos('  title button linknew image-text jslink ', ' ' .
                $this->fields[$r]['Type']. ' ') === false)) {
            ...
            if (!isset($this->style[$r]['showInTable'])) {
                if (!(strpos('  title button endgrid2 submit password ', ' ' . $this->fields[$r]['Type']. ' ')
                    === false)) {
                    $this->style[$r]['showInTable']='0';
                } else {
    ...
    function renderTable($block='', $fields='')
    {
        ...
        if (($block ==='') || ($block==='content')) {
            ...
            $this->tpl->assign('pagedTable_Id', $this->id);
            $this->tpl->assign('pagedTable_Name', $this->name);
            $this->tpl->assign("pagedTable_JS" ,
                "{$this->id}.element=document.getElementById('pagedtable[{$this->id}]');");
            $this->renderTitle();
            //Render rows
            if ($this->criteria->getDbName() == 'dbarray') {
            ...
            for ($j=0; $j < $rs->getRecordCount(); $j++) {
                ...
                if (count($this->masterdetail)>0) {
                    ...
                    foreach ($this->masterdetail as $keyMasterDetail => $fieldMasterDetail) {
                        if ($breakField[$fieldMasterDetail]!=$result[$fieldMasterDetail]) {
                            ...
                            $this->tpl->assign("value" ,
                                $this->fields[$fieldMasterDetail]['Label']==""?"":
                                $this->fields[$fieldMasterDetail]['Label'].": ".
                                $this->xmlForm->fields[ $fieldMasterDetail ]->renderTable($result[$fieldMasterDetail],
                                    $this->xmlForm, true));
                            ...
                            $this->tpl->assign("masterRowClass", $keyMasterDetail==0?"masterDetailMain":
                                "masterDetailOther");
                        }
                    }
                ...
                foreach ($this->fields as $r => $rval) {
                    if (strcasecmp($this->fields[$r]['Type'],'cellMark')==0) {
                        ...
                        $this->tdStyle = $this->xmlForm->fields[ $this->fields[$r]['Name'] ]->tdStyle($result1 ,
                            $this->xmlForm);
                        $this->tdClass = $this->xmlForm->fields[ $this->fields[$r]['Name'] ]->tdClass($result1 ,
                            $this->xmlForm);
            ...
            if ($this->currentPage > 1) {
                ...
                $first = "<a href=\"" . htmlentities($firstUrl  , ENT_QUOTES , 'utf-8') .
                    "\" onclick=\"".$firstAjax."\" class='firstPage'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a>";
                $prev  = "<a href=\"" . htmlentities($prevUrl  , ENT_QUOTES , 'utf-8') .
                    "\"  onclick=\"".$prevAjax."\" class='previousPage'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a>";
            } else {
            ...
            if ($this->currentPage < $this->totPages) {
                ...
                $next = "<a href=\"" . htmlentities($nextUrl   , ENT_QUOTES , 'utf-8') .
                    "\" onclick=\"".$nextAjax."\" class='nextPage'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a>";
                $last = "<a href=\"" . htmlentities($lastUrl   , ENT_QUOTES , 'utf-8') .
                    "\" onclick=\"".$lastAjax."\" class='lastPage'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a>";
            } else {
            ...
            for ($r=1; $r<=$this->totPages; $r++) {
                if (($r>=($this->currentPage-5)) && ($r<=($this->currentPage+5))) {
                    ...
                    if ($r!=$this->currentPage) {
                        $pagesEnum.="&nbsp;<a href=\"" . htmlentities($this->ownerPage . '?order=' .
                            $this->orderBy . '&page=' . $r  , ENT_QUOTES , 'utf-8') . "\" onclick=\"".
                            $pageAjax."\">".$r."</a>";
                    } else {
                        $pagesEnum.="&nbsp;<a>".$r."</a>";
                    }
            ...
            if (!$this->disableFooter) {
                ...
                if ($this->addRow) {
                    if ($this->sqlInsert != '') {
                        $this->tpl->assign("insert" , '<a href="#" onclick="pagedTable.event=\'Insert\';popup(\''.
                            $this->popupPage.'\');return false;">'./*G::LoadXml('labels','ID_ADD_NEW')*/ 'ID_ADD_NEW' .
                            '</a>');
                    }
                }
