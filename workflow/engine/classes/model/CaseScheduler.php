...
class CaseScheduler extends BaseCaseScheduler
{
    ...
    function caseSchedulerCron($date, &$log=array())
    {
        ...
        while ($aRow = $oDataset->getRow ()) {
            ...
            // note added consider the posibility to encapsulate some in functionality in a class method or some
            //funtions
            if ($sActualDataHour < $dActualSysHour) {
                ...
                if ($result->status_code == 0) {
                    ...
                    if ((isset($aRow['CASE_SH_PLUGIN_UID']))&&($aRow['CASE_SH_PLUGIN_UID']!="")) {
                        ...
                        if (count($pluginParts) == 2) {
                            ...
                            foreach ($activePluginsForCaseScheduler as $key => $caseSchedulerPlugin) {
                                if ((isset($caseSchedulerPlugin->sNamespace))
                                    &&($caseSchedulerPlugin->sNamespace==$pluginParts[0])
                                    &&(isset($caseSchedulerPlugin->sActionId))
                                    &&($caseSchedulerPlugin->sActionId==$pluginParts[1])) {
                    ...
                    if ((isset($caseSchedulerSelected))&&(is_object($caseSchedulerSelected))) {
                        eprintln(" - Transfering control to a Plugin: ".$caseSchedulerSelected->sNamespace."/".
                            $caseSchedulerSelected->sActionId,'green');
                        $oData['OBJ_SOAP'] = $client;
                        $oData['SCH_UID'] = $aRow['SCH_UID'];
                        $oData['params'] = $params;
                        $oData['sessionId'] = $sessionId;
                        $oData['userId'] = $user;
                        $paramsLogResultFromPlugin=$oPluginRegistry->executeMethod($caseSchedulerSelected->sNamespace,
                            $caseSchedulerSelected->sActionExecute, $oData);
                        ...
                        if ($result->status_code == 0) {
                            ...
                            $paramsLog ['WS_CREATE_CASE_STATUS'] = "Case " . $caseNumber . " " .
                ...
                if ($sOption != '4') {
                    ...
                    $dEstimatedDate = $this->updateNextRun ($sOption, $sValue, $sActualTime, $sDaysPerformTask,
                        $sWeeks, $sStartDay, $sMonths);

                    if ($aRow ['SCH_END_DATE'] != '') {
                        if (date ("Y-m-d", strtotime ($dEstimatedDate)) > date ("Y-m-d",
                            strtotime ($aRow ['SCH_END_DATE']))) {
                            $Fields = $this->Load ($sSchedulerUid);
                ...
                //$defaultEndpoint = 'http://' . $_SERVER ['SERVER_NAME'] . ':' . $_PORT . '/sys' . SYS_SYS .
        ...
        switch ($sOption) {
            case '1':
                switch ($sValue) {
                    ...
                    case '2':
                        ...
                        break;
                    case '3':
                        $dEstimatedDate = date ('Y-m-d  H:i:s', strtotime ("$nActualDate + " . $sDaysPerformTask .
                            " day"));
                        break;
                }
                break;
            case '2':
                if (strlen ($sWeeks) > 0) {
                    ...
                    $aDaysWeek = array ('Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday',
                        'Sunday');
                    ...
                    //die;
                    if ($nSW == 1) {
                        $dEstimatedDate = date ('Y-m-d', strtotime ("$nActualDate next " . $aDaysWeek [$nNextDay])) .
                            ' ' . date ('H:i:s', strtotime ($sActualTime));
                    } else {
                        ...
                        if ($nEveryDays == 1) {
                            $dEstimatedDate = date ('Y-m-d', strtotime ("$nActualDate " . $sTypeOperation . " " .
                                $aDaysWeek [$nFirstDay])) . ' ' . date ('H:i:s', strtotime ($sActualTime));
                        } else {
                            $nEveryDays = 1;
                            $nDataTmp = date ('Y-m-d', strtotime ("$nActualDate + " . $nEveryDays . " Week"));
                            $dEstimatedDate = date ('Y-m-d', strtotime ("$nDataTmp " . $sTypeOperation . " " .
                                $aDaysWeek [$nFirstDay])) . ' ' . date ('H:i:s', strtotime ($sActualTime));
                        }
                    }
                }
                ...
                if (strlen ($sMonths) > 0) {
                    ...
                    switch ($sValue) {
                        ...
                        case '1':
                            ...
                            $dEstimatedDate = date ('Y-m-d', strtotime ("$nYear-$nExecNextMonth-$nCurrentDay")) .
                                ' ' . date ('H:i:s', strtotime ($sActualTime));
                            break;
                        case '2':
                            $aMontsShort = array(
                            ...
                            $dEstimatedDate = date ('Y-m-d', strtotime ($sDaysWeekOpt . ' week ' .
                                $aWeeksShort [$sDayWeek - 1] . ' ' . $aMontsShort [$nExecNextMonth] . ' ' .
                                $nYear)) . ' ' . date ('H:i:s', strtotime ($sActualTime));
                            // krumo($nExecNextMonth, $sDayWeek);
                            // krumo($sDaysWeekOpt . ' week ' . $aWeeksShort[$sDayWeek-1] . ' ' .
                            //$aMontsShort[$nExecNextMonth] . ' ' . $nYear);
                            break;
                    }
                }
                break;
