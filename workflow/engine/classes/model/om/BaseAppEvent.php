...
abstract class BaseAppEvent extends BaseObject  implements Persistent
{
	...
	public function applyDefaultValues()
	{
		...
		$this->del_index = 0;
		$this->evn_uid = '';
		$this->app_evn_attempts = 0;
		$this->app_evn_status = 'OPEN';
	}

	/**
	 * Initializes internal state of BaseAppEvent object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [optionally formatted] temporal [app_evn_action_date] column value.
	 * 
	 *
	...
	{
		if ($this->app_evn_action_date === null) {
			return null;
		}


		if ($this->app_evn_action_date === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->app_evn_action_date);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->app_evn_action_date, true), $x);
			}
		}
	...
	/**
	 * Get the [optionally formatted] temporal [app_evn_last_execution_date] column value.
	 * 
	 *
	 * @param      string $format The date/time format string (either date()-style or strftime()-style).
	 *							If format is NULL, then the raw DateTime object will be returned.
	...
	{
		if ($this->app_evn_last_execution_date === null) {
			return null;
		}


		if ($this->app_evn_last_execution_date === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->app_evn_last_execution_date);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->app_evn_last_execution_date, true), $x);
			}
		}
	 ...
	 * @return     AppEvent The current object (for fluent API support)
	 */
	...
	/**
	 * Sets the value of [app_evn_action_date] column to a normalized version of the date/time value specified.
	 * 
	...
	public function setAppEvnActionDate($v)
	{
		...
		if ($this->app_evn_action_date !== null || $dt !== null) {
			$currentDateAsString = ($this->app_evn_action_date !== null && $tmpDt = new DateTime($this->app_evn_action_date)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newDateAsString = $dt ? $dt->format('Y-m-d H:i:s') : null;
			if ($currentDateAsString !== $newDateAsString) {
				$this->app_evn_action_date = $newDateAsString;
	...
	/**
	 * Sets the value of [app_evn_last_execution_date] column to a normalized version of the date/time value specified.
	 * 
	 * @param      mixed $v string, integer (timestamp), or DateTime value.
	 *               Empty strings are treated as NULL.
	 * @return     AppEvent The current object (for fluent API support)
	...
	{
		$dt = PropelDateTime::newInstance($v, null, 'DateTime');
		if ($this->app_evn_last_execution_date !== null || $dt !== null) {
			$currentDateAsString = ($this->app_evn_last_execution_date !== null && $tmpDt = new DateTime($this->app_evn_last_execution_date)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newDateAsString = $dt ? $dt->format('Y-m-d H:i:s') : null;
			if ($currentDateAsString !== $newDateAsString) {
				$this->app_evn_last_execution_date = $newDateAsString;
	...
	public function hasOnlyDefaultValues()
	{
			...
			if ($this->app_evn_attempts !== 0) {
				return false;
			}

			if ($this->app_evn_status !== 'OPEN') {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->evn_uid = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->app_evn_action_date = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->app_evn_attempts = ($row[$startcol + 4] !== null) ? (int) $row[$startcol + 4] : null;
			$this->app_evn_last_execution_date = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			$this->app_evn_status = ($row[$startcol + 6] !== null) ? (string) $row[$startcol + 6] : null;
			$this->resetModified();
			...
			return $startcol + 7; // 7 = AppEventPeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = AppEventPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = AppEventQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				AppEventPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	{
		if (isset($alreadyDumpedObjects['AppEvent'][serialize($this->getPrimaryKey())])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['AppEvent'][serialize($this->getPrimaryKey())] = true;
	...
	{
		return (null === $this->getAppUid()) && (null === $this->getDelIndex()) && (null === $this->getEvnUid());
	}
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setAppEvnActionDate($this->getAppEvnActionDate());
		$copyObj->setAppEvnAttempts($this->getAppEvnAttempts());
		$copyObj->setAppEvnLastExecutionDate($this->getAppEvnLastExecutionDate());
		$copyObj->setAppEvnStatus($this->getAppEvnStatus());
		if ($makeNew) {
			$copyObj->setNew(true);
		}
	}
	...
	public function clear()
	{
		...
		$this->del_index = null;
		$this->evn_uid = null;
		$this->app_evn_action_date = null;
		$this->app_evn_attempts = null;
		$this->app_evn_last_execution_date = null;
		$this->app_evn_status = null;
		$this->alreadyInSave = false;
		$this->alreadyInValidation = false;
	...
	{
		return (string) $this->exportTo(AppEventPeer::DEFAULT_STRING_FORMAT);
	}
