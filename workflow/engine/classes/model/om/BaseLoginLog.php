...
abstract class BaseLoginLog extends BaseObject  implements Persistent
{
	...
	public function applyDefaultValues()
	{
		...
		$this->log_ip = '';
		$this->log_sid = '';
		$this->log_client_hostname = '';
		$this->usr_uid = '';
	}

	/**
	 * Initializes internal state of BaseLoginLog object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [optionally formatted] temporal [log_init_date] column value.
	 * 
	 *
	...
	{
		if ($this->log_init_date === null) {
			return null;
		}


		if ($this->log_init_date === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->log_init_date);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->log_init_date, true), $x);
			}
		}
	...
	/**
	 * Get the [optionally formatted] temporal [log_end_date] column value.
	 * 
	 *
	 * @param      string $format The date/time format string (either date()-style or strftime()-style).
	 *							If format is NULL, then the raw DateTime object will be returned.
	...
	{
		if ($this->log_end_date === null) {
			return null;
		}


		if ($this->log_end_date === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->log_end_date);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->log_end_date, true), $x);
			}
		}
	...
	/**
	 * Sets the value of [log_init_date] column to a normalized version of the date/time value specified.
	 * 
	...
	public function setLogInitDate($v)
	{
		...
		if ($this->log_init_date !== null || $dt !== null) {
			$currentDateAsString = ($this->log_init_date !== null && $tmpDt = new DateTime($this->log_init_date)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newDateAsString = $dt ? $dt->format('Y-m-d H:i:s') : null;
			if ($currentDateAsString !== $newDateAsString) {
				$this->log_init_date = $newDateAsString;
	...
	/**
	 * Sets the value of [log_end_date] column to a normalized version of the date/time value specified.
	 * 
	 * @param      mixed $v string, integer (timestamp), or DateTime value.
	 *               Empty strings are treated as NULL.
	 * @return     LoginLog The current object (for fluent API support)
	...
	{
		$dt = PropelDateTime::newInstance($v, null, 'DateTime');
		if ($this->log_end_date !== null || $dt !== null) {
			$currentDateAsString = ($this->log_end_date !== null && $tmpDt = new DateTime($this->log_end_date)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newDateAsString = $dt ? $dt->format('Y-m-d H:i:s') : null;
			if ($currentDateAsString !== $newDateAsString) {
				$this->log_end_date = $newDateAsString;
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->log_uid = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			$this->log_status = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->log_ip = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->log_sid = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->log_init_date = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->log_end_date = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			$this->log_client_hostname = ($row[$startcol + 6] !== null) ? (string) $row[$startcol + 6] : null;
			$this->usr_uid = ($row[$startcol + 7] !== null) ? (string) $row[$startcol + 7] : null;
			$this->resetModified();
			...
			return $startcol + 8; // 8 = LoginLogPeer::NUM_HYDRATE_COLUMNS.
	...
	{
		$copyObj->setLogUid($this->getLogUid());
		$copyObj->setLogStatus($this->getLogStatus());
		$copyObj->setLogIp($this->getLogIp());
		$copyObj->setLogSid($this->getLogSid());
		$copyObj->setLogInitDate($this->getLogInitDate());
		$copyObj->setLogEndDate($this->getLogEndDate());
		$copyObj->setLogClientHostname($this->getLogClientHostname());
	...
	public function clear()
	{
		...
		$this->log_ip = null;
		$this->log_sid = null;
		$this->log_init_date = null;
		$this->log_end_date = null;
		$this->log_client_hostname = null;
		$this->usr_uid = null;
		$this->alreadyInSave = false;
		$this->alreadyInValidation = false;
