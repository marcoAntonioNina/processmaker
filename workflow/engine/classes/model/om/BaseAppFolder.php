...
abstract class BaseAppFolder extends BaseObject  implements Persistent
{
	...
	{
		$this->folder_uid = '';
		$this->folder_parent_uid = '';
	}

	/**
	 * Initializes internal state of BaseAppFolder object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [optionally formatted] temporal [folder_create_date] column value.
	 * 
	 *
	...
	{
		if ($this->folder_create_date === null) {
			return null;
		}


		if ($this->folder_create_date === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->folder_create_date);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->folder_create_date, true), $x);
			}
		}
	...
	/**
	 * Get the [optionally formatted] temporal [folder_update_date] column value.
	 * 
	 *
	 * @param      string $format The date/time format string (either date()-style or strftime()-style).
	 *							If format is NULL, then the raw DateTime object will be returned.
	...
	{
		if ($this->folder_update_date === null) {
			return null;
		}


		if ($this->folder_update_date === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->folder_update_date);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->folder_update_date, true), $x);
			}
		}
	 ...
	 * @return     AppFolder The current object (for fluent API support)
	 */
	...
	/**
	 * Sets the value of [folder_create_date] column to a normalized version of the date/time value specified.
	 * 
	...
	public function setFolderCreateDate($v)
	{
		...
		if ($this->folder_create_date !== null || $dt !== null) {
			$currentDateAsString = ($this->folder_create_date !== null && $tmpDt = new DateTime($this->folder_create_date)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newDateAsString = $dt ? $dt->format('Y-m-d H:i:s') : null;
			if ($currentDateAsString !== $newDateAsString) {
				$this->folder_create_date = $newDateAsString;
	...
	/**
	 * Sets the value of [folder_update_date] column to a normalized version of the date/time value specified.
	 * 
	 * @param      mixed $v string, integer (timestamp), or DateTime value.
	 *               Empty strings are treated as NULL.
	 * @return     AppFolder The current object (for fluent API support)
	...
	{
		$dt = PropelDateTime::newInstance($v, null, 'DateTime');
		if ($this->folder_update_date !== null || $dt !== null) {
			$currentDateAsString = ($this->folder_update_date !== null && $tmpDt = new DateTime($this->folder_update_date)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newDateAsString = $dt ? $dt->format('Y-m-d H:i:s') : null;
			if ($currentDateAsString !== $newDateAsString) {
				$this->folder_update_date = $newDateAsString;
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->folder_uid = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			$this->folder_parent_uid = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->folder_name = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->folder_create_date = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->folder_update_date = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->resetModified();
			...
			return $startcol + 5; // 5 = AppFolderPeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = AppFolderPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = AppFolderQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				AppFolderPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	{
		if (isset($alreadyDumpedObjects['AppFolder'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['AppFolder'][$this->getPrimaryKey()] = true;
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setFolderParentUid($this->getFolderParentUid());
		$copyObj->setFolderName($this->getFolderName());
		$copyObj->setFolderCreateDate($this->getFolderCreateDate());
		$copyObj->setFolderUpdateDate($this->getFolderUpdateDate());
		if ($makeNew) {
			$copyObj->setNew(true);
		}
	}
	...
	public function clear()
	{
		...
		$this->folder_parent_uid = null;
		$this->folder_name = null;
		$this->folder_create_date = null;
		$this->folder_update_date = null;
		$this->alreadyInSave = false;
		$this->alreadyInValidation = false;
	...
	{
		return (string) $this->exportTo(AppFolderPeer::DEFAULT_STRING_FORMAT);
	}
