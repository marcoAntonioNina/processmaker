...
abstract class BaseTranslation extends BaseObject  implements Persistent
{
	...
	/**
	 * Get the [optionally formatted] temporal [trn_update_date] column value.
	 * 
	 *
	 ...
	 */
	public function getTrnUpdateDate($format = '%x')
	{
		if ($this->trn_update_date === null) {
			return null;
		}


		if ($this->trn_update_date === '0000-00-00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->trn_update_date);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->trn_update_date, true), $x);
			}
		}
	...
	/**
	 * Sets the value of [trn_update_date] column to a normalized version of the date/time value specified.
	 * 
	...
	public function setTrnUpdateDate($v)
	{
		...
		if ($this->trn_update_date !== null || $dt !== null) {
			$currentDateAsString = ($this->trn_update_date !== null && $tmpDt = new DateTime($this->trn_update_date)) ? $tmpDt->format('Y-m-d') : null;
			$newDateAsString = $dt ? $dt->format('Y-m-d') : null;
			if ($currentDateAsString !== $newDateAsString) {
				$this->trn_update_date = $newDateAsString;
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->trn_category = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			$this->trn_id = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->trn_lang = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->trn_value = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->trn_update_date = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->resetModified();
			...
			return $startcol + 5; // 5 = TranslationPeer::NUM_HYDRATE_COLUMNS.
	...
	{
		return (null === $this->getTrnCategory()) && (null === $this->getTrnId()) && (null === $this->getTrnLang());
	}
	...
	{
		$copyObj->setTrnCategory($this->getTrnCategory());
		$copyObj->setTrnId($this->getTrnId());
		$copyObj->setTrnLang($this->getTrnLang());
		$copyObj->setTrnValue($this->getTrnValue());
		$copyObj->setTrnUpdateDate($this->getTrnUpdateDate());
		if ($makeNew) {
			$copyObj->setNew(true);
		}
	}
	...
	{
		$this->trn_category = null;
		$this->trn_id = null;
		$this->trn_lang = null;
		$this->trn_value = null;
		$this->trn_update_date = null;
		$this->alreadyInSave = false;
		$this->alreadyInValidation = false;
