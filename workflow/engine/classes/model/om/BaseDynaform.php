...
abstract class BaseDynaform extends BaseObject  implements Persistent
{
	 ...
	 * Note: this column has a database default value of: 'xmlform'
	 * @var        string
	 */
	protected $dyn_type;
	...
	public function applyDefaultValues()
	{
		$this->dyn_uid = '';
		$this->pro_uid = '0';
		$this->dyn_type = 'xmlform';
		$this->dyn_filename = '';
	}

	/**
	 * Initializes internal state of BaseDynaform object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     Dynaform The current object (for fluent API support)
	 */
	...
	public function hasOnlyDefaultValues()
	{
			...
			if ($this->dyn_type !== 'xmlform') {
				return false;
			}

			if ($this->dyn_filename !== '') {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->dyn_uid = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			$this->pro_uid = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->dyn_type = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->dyn_filename = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->resetModified();
			...
			return $startcol + 4; // 4 = DynaformPeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = DynaformPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = DynaformQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				DynaformPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	{
		if (isset($alreadyDumpedObjects['Dynaform'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Dynaform'][$this->getPrimaryKey()] = true;
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setDynType($this->getDynType());
		$copyObj->setDynFilename($this->getDynFilename());
		if ($makeNew) {
			$copyObj->setNew(true);
		}
	}
	...
	{
		return (string) $this->exportTo(DynaformPeer::DEFAULT_STRING_FORMAT);
	}
