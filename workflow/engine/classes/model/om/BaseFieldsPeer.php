...
require_once 'classes/model/map/FieldsTableMap.php';
...
abstract class BaseFieldsPeer
{
    ...
    /**
     * An identiy map to hold any loaded instances of Fields objects.
    ...
    protected static $fieldNames = array (
        ...
        BasePeer::TYPE_STUDLYPHPNAME => array ('fldUid', 'addTabUid', 'fldIndex', 'fldName', 'fldDescription', 'fldType', 'fldSize', 'fldNull', 'fldAutoIncrement', 'fldKey', 'fldForeignKey', 'fldForeignKeyTable', 'fldDynName', 'fldDynUid', 'fldFilter', ),
        BasePeer::TYPE_COLNAME => array (self::FLD_UID, self::ADD_TAB_UID, self::FLD_INDEX, self::FLD_NAME, self::FLD_DESCRIPTION, self::FLD_TYPE, self::FLD_SIZE, self::FLD_NULL, self::FLD_AUTO_INCREMENT, self::FLD_KEY, self::FLD_FOREIGN_KEY, self::FLD_FOREIGN_KEY_TABLE, self::FLD_DYN_NAME, self::FLD_DYN_UID, self::FLD_FILTER, ),
        BasePeer::TYPE_RAW_COLNAME => array ('FLD_UID', 'ADD_TAB_UID', 'FLD_INDEX', 'FLD_NAME', 'FLD_DESCRIPTION', 'FLD_TYPE', 'FLD_SIZE', 'FLD_NULL', 'FLD_AUTO_INCREMENT', 'FLD_KEY', 'FLD_FOREIGN_KEY', 'FLD_FOREIGN_KEY_TABLE', 'FLD_DYN_NAME', 'FLD_DYN_UID', 'FLD_FILTER', ),
    ...
    protected static $fieldKeys = array (
        ...
        BasePeer::TYPE_STUDLYPHPNAME => array ('fldUid' => 0, 'addTabUid' => 1, 'fldIndex' => 2, 'fldName' => 3, 'fldDescription' => 4, 'fldType' => 5, 'fldSize' => 6, 'fldNull' => 7, 'fldAutoIncrement' => 8, 'fldKey' => 9, 'fldForeignKey' => 10, 'fldForeignKeyTable' => 11, 'fldDynName' => 12, 'fldDynUid' => 13, 'fldFilter' => 14, ),
        BasePeer::TYPE_COLNAME => array (self::FLD_UID => 0, self::ADD_TAB_UID => 1, self::FLD_INDEX => 2, self::FLD_NAME => 3, self::FLD_DESCRIPTION => 4, self::FLD_TYPE => 5, self::FLD_SIZE => 6, self::FLD_NULL => 7, self::FLD_AUTO_INCREMENT => 8, self::FLD_KEY => 9, self::FLD_FOREIGN_KEY => 10, self::FLD_FOREIGN_KEY_TABLE => 11, self::FLD_DYN_NAME => 12, self::FLD_DYN_UID => 13, self::FLD_FILTER => 14, ),
        BasePeer::TYPE_RAW_COLNAME => array ('FLD_UID' => 0, 'ADD_TAB_UID' => 1, 'FLD_INDEX' => 2, 'FLD_NAME' => 3, 'FLD_DESCRIPTION' => 4, 'FLD_TYPE' => 5, 'FLD_SIZE' => 6, 'FLD_NULL' => 7, 'FLD_AUTO_INCREMENT' => 8, 'FLD_KEY' => 9, 'FLD_FOREIGN_KEY' => 10, 'FLD_FOREIGN_KEY_TABLE' => 11, 'FLD_DYN_NAME' => 12, 'FLD_DYN_UID' => 13, 'FLD_FILTER' => 14, ),
    ...
    public static function addSelectColumns(Criteria $criteria, $alias = null)
    {
        if (null === $alias) {
            ...
            $criteria->addSelectColumn($alias . '.FLD_UID');
            $criteria->addSelectColumn($alias . '.ADD_TAB_UID');
            $criteria->addSelectColumn($alias . '.FLD_INDEX');
            $criteria->addSelectColumn($alias . '.FLD_NAME');
            $criteria->addSelectColumn($alias . '.FLD_DESCRIPTION');
            $criteria->addSelectColumn($alias . '.FLD_TYPE');
            $criteria->addSelectColumn($alias . '.FLD_SIZE');
            $criteria->addSelectColumn($alias . '.FLD_NULL');
            $criteria->addSelectColumn($alias . '.FLD_AUTO_INCREMENT');
            $criteria->addSelectColumn($alias . '.FLD_KEY');
            $criteria->addSelectColumn($alias . '.FLD_FOREIGN_KEY');
            $criteria->addSelectColumn($alias . '.FLD_FOREIGN_KEY_TABLE');
            $criteria->addSelectColumn($alias . '.FLD_DYN_NAME');
            $criteria->addSelectColumn($alias . '.FLD_DYN_UID');
            $criteria->addSelectColumn($alias . '.FLD_FILTER');
        }
    }
    ...
    public static function doCount(Criteria $criteria, $distinct = false, PropelPDO $con = null)
    {
        ...
        $criteria->setPrimaryTableName(FieldsPeer::TABLE_NAME);
    ...
    {
        return FieldsPeer::populateObjects(FieldsPeer::doSelectStmt($criteria, $con));
    }
    /**
     ...
     *
     * @param      Fields $value A Fields object.
     ...
     *
     * @param      mixed $value A Fields object or a primary key value.
     */
    public static function removeInstanceFromPool($value)
    {
        if (Propel::isInstancePoolingEnabled() && $value !== null) {
            if (is_object($value) && $value instanceof Fields) {
                $key = (string) $value->getFldUid();
            } elseif (is_scalar($value)) {
                // assume we've been passed a primary key
                $key = (string) $value;
            } else {
                $e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or Fields object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value,true)));
                throw $e;
            }
     ...
     * @return     Fields Found object or NULL if 1) no instance exists for specified key or 2) instance pooling has been disabled.
    ...
    public static function populateObjects(PDOStatement $stmt)
    {
        ...
        $cls = FieldsPeer::getOMClass(false);
        // populate the object(s)
        while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
            $key = FieldsPeer::getPrimaryKeyHashFromRow($row, 0);
            if (null !== ($obj = FieldsPeer::getInstanceFromPool($key))) {
                ...
                $obj->hydrate($row);
                $results[] = $obj;
                FieldsPeer::addInstanceToPool($obj, $key);
            } // if key exists
        }
        $stmt->closeCursor();
        return $results;
    ...
    {
        $key = FieldsPeer::getPrimaryKeyHashFromRow($row, $startcol);
        if (null !== ($obj = FieldsPeer::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $startcol, true); // rehydrate
            $col = $startcol + FieldsPeer::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = FieldsPeer::OM_CLASS;
            $obj = new $cls();
    ...
    {
        $dbMap = Propel::getDatabaseMap(BaseFieldsPeer::DATABASE_NAME);
        if (!$dbMap->hasTable(BaseFieldsPeer::TABLE_NAME)) {
    ...
    {
        return $withPrefix ? FieldsPeer::CLASS_DEFAULT : FieldsPeer::OM_CLASS;
    }
    ...
    /**
     * Performs an UPDATE on the database, given a Fields or Criteria object.
     *
    ...
    public static function doUpdate($values, PropelPDO $con = null)
    {
        ...
        if ($values instanceof Criteria) {
            ...
            $value = $criteria->remove(FieldsPeer::FLD_UID);
            if ($value) {
                $selectCriteria->add(FieldsPeer::FLD_UID, $value, $comparison);
            } else {
                $selectCriteria->setPrimaryTableName(FieldsPeer::TABLE_NAME);
            }

        } else {
            // $values is Fields object
    ...
    public static function doDeleteAll(PropelPDO $con = null)
    {
        ...
        try {
            ...
            $con->beginTransaction();
            $affectedRows += BasePeer::doDeleteAll(FieldsPeer::TABLE_NAME, $con, FieldsPeer::DATABASE_NAME);
            // Because this db requires some delete cascade/set null emulation, we have to
            // clear the cached instance *after* the emulation has happened (since
            // instances get re-added by the select statement contained therein).
            FieldsPeer::clearInstancePool();
            FieldsPeer::clearRelatedInstancePool();
            $con->commit();
            return $affectedRows;
        } catch (PropelException $e) {
            $con->rollBack();
    ...
    public static function doDelete($values, PropelPDO $con = null)
    {
        ...
        if ($values instanceof Criteria) {
        ...
        } elseif ($values instanceof Fields) { // it's a model object
            // invalidate the cache for this single object
            FieldsPeer::removeInstanceFromPool($values);
            ...
            foreach ((array) $values as $singleval) {
                FieldsPeer::removeInstanceFromPool($singleval);
            }
        }
    ...
    public static function retrieveByPK($pk, PropelPDO $con = null)
    {
        ...
        if (null !== ($obj = FieldsPeer::getInstanceFromPool((string) $pk))) {
            return $obj;
        }
