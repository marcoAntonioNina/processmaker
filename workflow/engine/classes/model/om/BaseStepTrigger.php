...
abstract class BaseStepTrigger extends BaseObject  implements Persistent
{
	...
	/**
	 * Initializes internal state of BaseStepTrigger object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     StepTrigger The current object (for fluent API support)
	 */
	...
	public function hasOnlyDefaultValues()
	{
			...
			if ($this->st_condition !== '') {
				return false;
			}

			if ($this->st_position !== 0) {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->tri_uid = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->st_type = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->st_condition = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->st_position = ($row[$startcol + 5] !== null) ? (int) $row[$startcol + 5] : null;
			$this->resetModified();
			...
			return $startcol + 6; // 6 = StepTriggerPeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = StepTriggerPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = StepTriggerQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				StepTriggerPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	{
		if (isset($alreadyDumpedObjects['StepTrigger'][serialize($this->getPrimaryKey())])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['StepTrigger'][serialize($this->getPrimaryKey())] = true;
	...
	{
		return (null === $this->getStepUid()) && (null === $this->getTasUid()) && (null === $this->getTriUid()) && (null === $this->getStType());
	}
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setStType($this->getStType());
		$copyObj->setStCondition($this->getStCondition());
		$copyObj->setStPosition($this->getStPosition());
		if ($makeNew) {
			$copyObj->setNew(true);
		}
	}
	...
	public function clear()
	{
		...
		$this->tri_uid = null;
		$this->st_type = null;
		$this->st_condition = null;
		$this->st_position = null;
		$this->alreadyInSave = false;
		$this->alreadyInValidation = false;
	...
	{
		return (string) $this->exportTo(StepTriggerPeer::DEFAULT_STRING_FORMAT);
	}
