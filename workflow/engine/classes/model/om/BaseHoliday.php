...
abstract class BaseHoliday extends BaseObject  implements Persistent
{
	...
	{
		$this->hld_date = '0000-00-00';
		$this->hld_description = '';
	}
	...
	{
			if ($this->hld_date !== '0000-00-00') {
				return false;
			}

			if ($this->hld_description !== '') {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->hld_uid = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			$this->hld_date = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->hld_description = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->resetModified();
			...
			return $startcol + 3; // 3 = HolidayPeer::NUM_HYDRATE_COLUMNS.
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(HolidayPeer::HLD_UID) ) {
	...
	{
		$copyObj->setHldDate($this->getHldDate());
		$copyObj->setHldDescription($this->getHldDescription());
		if ($makeNew) {
			$copyObj->setNew(true);
			$copyObj->setHldUid(NULL); // this is a auto-increment column, so set to default value
		}
	}
