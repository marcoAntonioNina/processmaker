...
require_once 'classes/model/map/ObjectPermissionTableMap.php';
...
abstract class BaseObjectPermissionPeer
{
    ...
    const OM_CLASS = 'ObjectPermission';

    /** A class that can be returned by this peer. */
    const CLASS_DEFAULT = 'classes.model.ObjectPermission';

    /** the related TableMap class for this table */
    const TM_CLASS = 'ObjectPermissionTableMap';
    ...
    /**
     * An identiy map to hold any loaded instances of ObjectPermission objects.
     * This must be public so that other peer classes can access this when hydrating from JOIN
     * queries.
     * @var        array ObjectPermission[]
     */
    ...
    protected static $fieldNames = array (
        ...
        BasePeer::TYPE_STUDLYPHPNAME => array ('opUid', 'proUid', 'tasUid', 'usrUid', 'opUserRelation', 'opTaskSource', 'opParticipate', 'opObjType', 'opObjUid', 'opAction', 'opCaseStatus', ),
        BasePeer::TYPE_COLNAME => array (self::OP_UID, self::PRO_UID, self::TAS_UID, self::USR_UID, self::OP_USER_RELATION, self::OP_TASK_SOURCE, self::OP_PARTICIPATE, self::OP_OBJ_TYPE, self::OP_OBJ_UID, self::OP_ACTION, self::OP_CASE_STATUS, ),
        BasePeer::TYPE_RAW_COLNAME => array ('OP_UID', 'PRO_UID', 'TAS_UID', 'USR_UID', 'OP_USER_RELATION', 'OP_TASK_SOURCE', 'OP_PARTICIPATE', 'OP_OBJ_TYPE', 'OP_OBJ_UID', 'OP_ACTION', 'OP_CASE_STATUS', ),
    ...
    protected static $fieldKeys = array (
        ...
        BasePeer::TYPE_STUDLYPHPNAME => array ('opUid' => 0, 'proUid' => 1, 'tasUid' => 2, 'usrUid' => 3, 'opUserRelation' => 4, 'opTaskSource' => 5, 'opParticipate' => 6, 'opObjType' => 7, 'opObjUid' => 8, 'opAction' => 9, 'opCaseStatus' => 10, ),
        BasePeer::TYPE_COLNAME => array (self::OP_UID => 0, self::PRO_UID => 1, self::TAS_UID => 2, self::USR_UID => 3, self::OP_USER_RELATION => 4, self::OP_TASK_SOURCE => 5, self::OP_PARTICIPATE => 6, self::OP_OBJ_TYPE => 7, self::OP_OBJ_UID => 8, self::OP_ACTION => 9, self::OP_CASE_STATUS => 10, ),
        BasePeer::TYPE_RAW_COLNAME => array ('OP_UID' => 0, 'PRO_UID' => 1, 'TAS_UID' => 2, 'USR_UID' => 3, 'OP_USER_RELATION' => 4, 'OP_TASK_SOURCE' => 5, 'OP_PARTICIPATE' => 6, 'OP_OBJ_TYPE' => 7, 'OP_OBJ_UID' => 8, 'OP_ACTION' => 9, 'OP_CASE_STATUS' => 10, ),
    ...
    public static function addSelectColumns(Criteria $criteria, $alias = null)
    {
        if (null === $alias) {
            ...
            $criteria->addSelectColumn($alias . '.OP_UID');
            $criteria->addSelectColumn($alias . '.PRO_UID');
            $criteria->addSelectColumn($alias . '.TAS_UID');
            $criteria->addSelectColumn($alias . '.USR_UID');
            $criteria->addSelectColumn($alias . '.OP_USER_RELATION');
            $criteria->addSelectColumn($alias . '.OP_TASK_SOURCE');
            $criteria->addSelectColumn($alias . '.OP_PARTICIPATE');
            $criteria->addSelectColumn($alias . '.OP_OBJ_TYPE');
            $criteria->addSelectColumn($alias . '.OP_OBJ_UID');
            $criteria->addSelectColumn($alias . '.OP_ACTION');
            $criteria->addSelectColumn($alias . '.OP_CASE_STATUS');
        }
    }
    ...
    public static function doCount(Criteria $criteria, $distinct = false, PropelPDO $con = null)
    {
        ...
        $criteria->setPrimaryTableName(ObjectPermissionPeer::TABLE_NAME);
        ...
        if ($con === null) {
            $con = Propel::getConnection(ObjectPermissionPeer::DATABASE_NAME, Propel::CONNECTION_READ);
        }
    ...
    {
        return ObjectPermissionPeer::populateObjects(ObjectPermissionPeer::doSelectStmt($criteria, $con));
    }
    /**
     ...
     *
     * @param      ObjectPermission $value A ObjectPermission object.
     ...
     *
     * @param      mixed $value A ObjectPermission object or a primary key value.
     */
    public static function removeInstanceFromPool($value)
    {
        if (Propel::isInstancePoolingEnabled() && $value !== null) {
            if (is_object($value) && $value instanceof ObjectPermission) {
                $key = (string) $value->getOpUid();
            } elseif (is_scalar($value)) {
                // assume we've been passed a primary key
                $key = (string) $value;
            } else {
                $e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or ObjectPermission object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value,true)));
                throw $e;
            }
     ...
     * @return     ObjectPermission Found object or NULL if 1) no instance exists for specified key or 2) instance pooling has been disabled.
    ...
    /**
     * Method to invalidate the instance pool of all tables related to OBJECT_PERMISSION
    ...
    public static function populateObjects(PDOStatement $stmt)
    {
        ...
        $cls = ObjectPermissionPeer::getOMClass(false);
        // populate the object(s)
        while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
            $key = ObjectPermissionPeer::getPrimaryKeyHashFromRow($row, 0);
            if (null !== ($obj = ObjectPermissionPeer::getInstanceFromPool($key))) {
                ...
                $obj->hydrate($row);
                $results[] = $obj;
                ObjectPermissionPeer::addInstanceToPool($obj, $key);
            } // if key exists
        }
        $stmt->closeCursor();
        return $results;
     ...
     * @throws     PropelException Any exceptions caught during processing will be
     *             rethrown wrapped into a PropelException.
     * @return     array (ObjectPermission object, last column rank)
     */
    public static function populateObject($row, $startcol = 0)
    {
        $key = ObjectPermissionPeer::getPrimaryKeyHashFromRow($row, $startcol);
        if (null !== ($obj = ObjectPermissionPeer::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $startcol, true); // rehydrate
            $col = $startcol + ObjectPermissionPeer::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = ObjectPermissionPeer::OM_CLASS;
            $obj = new $cls();
    ...
    {
        $dbMap = Propel::getDatabaseMap(BaseObjectPermissionPeer::DATABASE_NAME);
        if (!$dbMap->hasTable(BaseObjectPermissionPeer::TABLE_NAME)) {
            $dbMap->addTableObject(new ObjectPermissionTableMap());
        }
    }
    ...
    {
        return $withPrefix ? ObjectPermissionPeer::CLASS_DEFAULT : ObjectPermissionPeer::OM_CLASS;
    }

    /**
     * Performs an INSERT on the database, given a ObjectPermission or Criteria object.
     *
    ...
    {
        if ($con === null) {
            $con = Propel::getConnection(ObjectPermissionPeer::DATABASE_NAME, Propel::CONNECTION_WRITE);
        }
    ...
    /**
     * Performs an UPDATE on the database, given a ObjectPermission or Criteria object.
     *
    ...
    public static function doUpdate($values, PropelPDO $con = null)
    {
        ...
        if ($values instanceof Criteria) {
            ...
            $value = $criteria->remove(ObjectPermissionPeer::OP_UID);
            if ($value) {
                $selectCriteria->add(ObjectPermissionPeer::OP_UID, $value, $comparison);
            } else {
                $selectCriteria->setPrimaryTableName(ObjectPermissionPeer::TABLE_NAME);
            }

        } else {
            // $values is ObjectPermission object
    ...
    /**
     * Deletes all rows from the OBJECT_PERMISSION table.
     *
     * @param      PropelPDO $con the connection to use
     * @return     int The number of affected rows (if supported by underlying database driver).
     */
    public static function doDeleteAll(PropelPDO $con = null)
    {
        ...
        try {
            ...
            $con->beginTransaction();
            $affectedRows += BasePeer::doDeleteAll(ObjectPermissionPeer::TABLE_NAME, $con, ObjectPermissionPeer::DATABASE_NAME);
            // Because this db requires some delete cascade/set null emulation, we have to
            // clear the cached instance *after* the emulation has happened (since
            // instances get re-added by the select statement contained therein).
            ObjectPermissionPeer::clearInstancePool();
            ObjectPermissionPeer::clearRelatedInstancePool();
            $con->commit();
            return $affectedRows;
        } catch (PropelException $e) {
            $con->rollBack();
    ...
    /**
     * Performs a DELETE on the database, given a ObjectPermission or Criteria object OR a primary key value.
     *
    ...
    public static function doDelete($values, PropelPDO $con = null)
    {
        ...
        if ($values instanceof Criteria) {
        ...
        } elseif ($values instanceof ObjectPermission) { // it's a model object
            // invalidate the cache for this single object
            ObjectPermissionPeer::removeInstanceFromPool($values);
            ...
            foreach ((array) $values as $singleval) {
                ObjectPermissionPeer::removeInstanceFromPool($singleval);
            }
        }
    ...
    public static function retrieveByPK($pk, PropelPDO $con = null)
    {
        ...
        if (null !== ($obj = ObjectPermissionPeer::getInstanceFromPool((string) $pk))) {
            return $obj;
        }
...
//
BaseObjectPermissionPeer::buildTableMap();
