...
abstract class BaseLexico extends BaseObject  implements Persistent
{
	...
	/**
	 * Initializes internal state of BaseLexico object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     Lexico The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->lex_topic = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			$this->lex_key = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->lex_value = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->lex_caption = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->resetModified();
			...
			return $startcol + 4; // 4 = LexicoPeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = LexicoPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = LexicoQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				LexicoPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	{
		if (isset($alreadyDumpedObjects['Lexico'][serialize($this->getPrimaryKey())])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Lexico'][serialize($this->getPrimaryKey())] = true;
	...
	{
		return (null === $this->getLexTopic()) && (null === $this->getLexKey());
	}
	...
	{
		$copyObj->setLexTopic($this->getLexTopic());
		$copyObj->setLexKey($this->getLexKey());
		$copyObj->setLexValue($this->getLexValue());
		$copyObj->setLexCaption($this->getLexCaption());
		if ($makeNew) {
			$copyObj->setNew(true);
		}
	}
	...
	{
		return (string) $this->exportTo(LexicoPeer::DEFAULT_STRING_FORMAT);
	}
