...
abstract class BaseIsoLocation extends BaseObject  implements Persistent
{
	...
	/**
	 * Initializes internal state of BaseIsoLocation object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     IsoLocation The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->ic_uid = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			$this->il_uid = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->il_name = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->il_normal_name = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->is_uid = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->resetModified();
			...
			return $startcol + 5; // 5 = IsoLocationPeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = IsoLocationPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = IsoLocationQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				IsoLocationPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	{
		if (isset($alreadyDumpedObjects['IsoLocation'][serialize($this->getPrimaryKey())])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['IsoLocation'][serialize($this->getPrimaryKey())] = true;
	...
	{
		return (null === $this->getIcUid()) && (null === $this->getIlUid());
	}
	...
	{
		$copyObj->setIcUid($this->getIcUid());
		$copyObj->setIlUid($this->getIlUid());
		$copyObj->setIlName($this->getIlName());
		$copyObj->setIlNormalName($this->getIlNormalName());
		$copyObj->setIsUid($this->getIsUid());
		if ($makeNew) {
			$copyObj->setNew(true);
		}
	}
	...
	{
		return (string) $this->exportTo(IsoLocationPeer::DEFAULT_STRING_FORMAT);
	}
