...
abstract class BaseUsersProperties extends BaseObject  implements Persistent
{
	 ...
	 * Peer class name
	 */
	const PEER = 'UsersPropertiesPeer';
	 ...
	 * @var        int
	 */
	protected $usr_logged_next_time;
	...
	{
		$this->usr_uid = '';
		$this->usr_logged_next_time = 0;
	}

	/**
	 * Initializes internal state of BaseUsersProperties object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [optionally formatted] temporal [usr_last_update_date] column value.
	 * 
	 *
	...
	{
		if ($this->usr_last_update_date === null) {
			return null;
		}


		if ($this->usr_last_update_date === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->usr_last_update_date);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->usr_last_update_date, true), $x);
			}
		}
	 ...
	 * @return     UsersProperties The current object (for fluent API support)
	 */
	...
	/**
	 * Sets the value of [usr_last_update_date] column to a normalized version of the date/time value specified.
	 * 
	...
	public function setUsrLastUpdateDate($v)
	{
		...
		if ($this->usr_last_update_date !== null || $dt !== null) {
			$currentDateAsString = ($this->usr_last_update_date !== null && $tmpDt = new DateTime($this->usr_last_update_date)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newDateAsString = $dt ? $dt->format('Y-m-d H:i:s') : null;
			if ($currentDateAsString !== $newDateAsString) {
				$this->usr_last_update_date = $newDateAsString;
	...
	public function setUsrLoggedNextTime($v)
	{
		...
		if ($this->usr_logged_next_time !== $v) {
	...
	public function hasOnlyDefaultValues()
	{
			...
			if ($this->usr_logged_next_time !== 0) {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->usr_last_update_date = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->usr_logged_next_time = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			$this->usr_password_history = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->resetModified();
			...
			return $startcol + 4; // 4 = UsersPropertiesPeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = UsersPropertiesPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = UsersPropertiesQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				UsersPropertiesPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	{
		if (isset($alreadyDumpedObjects['UsersProperties'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['UsersProperties'][$this->getPrimaryKey()] = true;
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setUsrLastUpdateDate($this->getUsrLastUpdateDate());
		$copyObj->setUsrLoggedNextTime($this->getUsrLoggedNextTime());
		$copyObj->setUsrPasswordHistory($this->getUsrPasswordHistory());
		if ($makeNew) {
			$copyObj->setNew(true);
		}
	}
	...
	{
		$this->usr_uid = null;
		$this->usr_last_update_date = null;
		$this->usr_logged_next_time = null;
		$this->usr_password_history = null;
		$this->alreadyInSave = false;
		$this->alreadyInValidation = false;
	...
	{
		return (string) $this->exportTo(UsersPropertiesPeer::DEFAULT_STRING_FORMAT);
	}
