...
abstract class BaseCalendarDefinition extends BaseObject  implements Persistent
{
	 ...
	 * Peer class name
	 */
	const PEER = 'CalendarDefinitionPeer';
	...
	public function applyDefaultValues()
	{
		$this->calendar_uid = '';
		$this->calendar_name = '';
		$this->calendar_work_days = '';
		$this->calendar_status = 'ACTIVE';
	}

	/**
	 * Initializes internal state of BaseCalendarDefinition object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [optionally formatted] temporal [calendar_create_date] column value.
	 * 
	 *
	...
	{
		if ($this->calendar_create_date === null) {
			return null;
		}


		if ($this->calendar_create_date === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->calendar_create_date);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->calendar_create_date, true), $x);
			}
		}
	...
	/**
	 * Get the [optionally formatted] temporal [calendar_update_date] column value.
	 * 
	 *
	 * @param      string $format The date/time format string (either date()-style or strftime()-style).
	 *							If format is NULL, then the raw DateTime object will be returned.
	...
	{
		if ($this->calendar_update_date === null) {
			return null;
		}


		if ($this->calendar_update_date === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->calendar_update_date);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->calendar_update_date, true), $x);
			}
		}
	 ...
	 * @return     CalendarDefinition The current object (for fluent API support)
	 */
	...
	/**
	 * Sets the value of [calendar_create_date] column to a normalized version of the date/time value specified.
	 * 
	...
	public function setCalendarCreateDate($v)
	{
		...
		if ($this->calendar_create_date !== null || $dt !== null) {
			$currentDateAsString = ($this->calendar_create_date !== null && $tmpDt = new DateTime($this->calendar_create_date)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newDateAsString = $dt ? $dt->format('Y-m-d H:i:s') : null;
			if ($currentDateAsString !== $newDateAsString) {
				$this->calendar_create_date = $newDateAsString;
	...
	/**
	 * Sets the value of [calendar_update_date] column to a normalized version of the date/time value specified.
	 * 
	 * @param      mixed $v string, integer (timestamp), or DateTime value.
	 *               Empty strings are treated as NULL.
	 * @return     CalendarDefinition The current object (for fluent API support)
	...
	{
		$dt = PropelDateTime::newInstance($v, null, 'DateTime');
		if ($this->calendar_update_date !== null || $dt !== null) {
			$currentDateAsString = ($this->calendar_update_date !== null && $tmpDt = new DateTime($this->calendar_update_date)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newDateAsString = $dt ? $dt->format('Y-m-d H:i:s') : null;
			if ($currentDateAsString !== $newDateAsString) {
				$this->calendar_update_date = $newDateAsString;
	...
	public function hasOnlyDefaultValues()
	{
			...
			if ($this->calendar_work_days !== '') {
				return false;
			}

			if ($this->calendar_status !== 'ACTIVE') {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->calendar_uid = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			$this->calendar_name = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->calendar_create_date = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->calendar_update_date = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->calendar_work_days = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->calendar_description = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			$this->calendar_status = ($row[$startcol + 6] !== null) ? (string) $row[$startcol + 6] : null;
			$this->resetModified();
			...
			return $startcol + 7; // 7 = CalendarDefinitionPeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = CalendarDefinitionPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = CalendarDefinitionQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				CalendarDefinitionPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	{
		if (isset($alreadyDumpedObjects['CalendarDefinition'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['CalendarDefinition'][$this->getPrimaryKey()] = true;
	...
	{
		return null === $this->getCalendarUid();
	}
	...
	{
		$copyObj->setCalendarUid($this->getCalendarUid());
		$copyObj->setCalendarName($this->getCalendarName());
		$copyObj->setCalendarCreateDate($this->getCalendarCreateDate());
		$copyObj->setCalendarUpdateDate($this->getCalendarUpdateDate());
		$copyObj->setCalendarWorkDays($this->getCalendarWorkDays());
		$copyObj->setCalendarDescription($this->getCalendarDescription());
		$copyObj->setCalendarStatus($this->getCalendarStatus());
		if ($makeNew) {
			$copyObj->setNew(true);
		}
	}
	...
	{
		$this->calendar_uid = null;
		$this->calendar_name = null;
		$this->calendar_create_date = null;
		$this->calendar_update_date = null;
		$this->calendar_work_days = null;
		$this->calendar_description = null;
		$this->calendar_status = null;
		$this->alreadyInSave = false;
		$this->alreadyInValidation = false;
	...
	{
		return (string) $this->exportTo(CalendarDefinitionPeer::DEFAULT_STRING_FORMAT);
	}
