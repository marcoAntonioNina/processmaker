...
abstract class BaseGroupwf extends BaseObject  implements Persistent
{
	...
	{
		$this->grp_uid = '';
		$this->grp_status = 'ACTIVE';
		$this->grp_ldap_dn = '';
		$this->grp_ux = 'NORMAL';
	}

	/**
	 * Initializes internal state of BaseGroupwf object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     Groupwf The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->grp_uid = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			$this->grp_status = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->grp_ldap_dn = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->grp_ux = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->resetModified();
			...
			return $startcol + 4; // 4 = GroupwfPeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = GroupwfPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = GroupwfQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				GroupwfPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	{
		if (isset($alreadyDumpedObjects['Groupwf'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Groupwf'][$this->getPrimaryKey()] = true;
	...
	{
		$copyObj->setGrpUid($this->getGrpUid());
		$copyObj->setGrpStatus($this->getGrpStatus());
		$copyObj->setGrpLdapDn($this->getGrpLdapDn());
		$copyObj->setGrpUx($this->getGrpUx());
		if ($makeNew) {
			$copyObj->setNew(true);
		}
	}
	...
	{
		$this->grp_uid = null;
		$this->grp_status = null;
		$this->grp_ldap_dn = null;
		$this->grp_ux = null;
		$this->alreadyInSave = false;
		$this->alreadyInValidation = false;
	...
	{
		return (string) $this->exportTo(GroupwfPeer::DEFAULT_STRING_FORMAT);
	}
