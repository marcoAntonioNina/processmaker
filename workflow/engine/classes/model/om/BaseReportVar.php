...
abstract class BaseReportVar extends BaseObject  implements Persistent
{
	...
	/**
	 * Initializes internal state of BaseReportVar object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     ReportVar The current object (for fluent API support)
	 */
	...
	public function hasOnlyDefaultValues()
	{
			...
			if ($this->rep_var_name !== '') {
				return false;
			}

			if ($this->rep_var_type !== '') {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->rep_var_uid = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			$this->pro_uid = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->rep_tab_uid = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->rep_var_name = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->rep_var_type = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->resetModified();
			...
			return $startcol + 5; // 5 = ReportVarPeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = ReportVarPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = ReportVarQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				ReportVarPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	{
		if (isset($alreadyDumpedObjects['ReportVar'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['ReportVar'][$this->getPrimaryKey()] = true;
	...
	{
		$copyObj->setRepVarUid($this->getRepVarUid());
		$copyObj->setProUid($this->getProUid());
		$copyObj->setRepTabUid($this->getRepTabUid());
		$copyObj->setRepVarName($this->getRepVarName());
		$copyObj->setRepVarType($this->getRepVarType());
		if ($makeNew) {
			$copyObj->setNew(true);
		}
	}
	...
	{
		$this->rep_var_uid = null;
		$this->pro_uid = null;
		$this->rep_tab_uid = null;
		$this->rep_var_name = null;
		$this->rep_var_type = null;
		$this->alreadyInSave = false;
		$this->alreadyInValidation = false;
	...
	{
		return (string) $this->exportTo(ReportVarPeer::DEFAULT_STRING_FORMAT);
	}
