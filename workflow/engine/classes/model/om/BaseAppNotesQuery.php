 ...
 * 
 *
 * @method     AppNotesQuery orderByNoteUid($order = Criteria::ASC) Order by the NOTE_UID column
 ...
 *
 * @method     AppNotesQuery groupByNoteUid() Group by the NOTE_UID column
 ...
 *
 * @method     AppNotes findOneByNoteUid(int $NOTE_UID) Return the first AppNotes filtered by the NOTE_UID column
 ...
 *
 * @method     array findByNoteUid(int $NOTE_UID) Return AppNotes objects filtered by the NOTE_UID column
...
abstract class BaseAppNotesQuery extends ModelCriteria
{
    ...
    {
        return $this->addUsingAlias(AppNotesPeer::NOTE_UID, $key, Criteria::EQUAL);
    }
    ...
    {
        return $this->addUsingAlias(AppNotesPeer::NOTE_UID, $keys, Criteria::IN);
    }

    /**
     * Filter the query on the NOTE_UID column
     *
     * Example usage:
     * <code>
     * $query->filterByNoteUid(1234); // WHERE NOTE_UID = 1234
     * $query->filterByNoteUid(array(12, 34)); // WHERE NOTE_UID IN (12, 34)
     * $query->filterByNoteUid(array('min' => 12)); // WHERE NOTE_UID > 12
     * </code>
     *
     * @param     mixed $noteUid The value to use as filter.
     ...
     * @return    AppNotesQuery The current query, for fluid interface
     */
    public function filterByNoteUid($noteUid = null, $comparison = null)
    {
        if (is_array($noteUid) && null === $comparison) {
            $comparison = Criteria::IN;
        }
        return $this->addUsingAlias(AppNotesPeer::NOTE_UID, $noteUid, $comparison);
    }
    ...
    {
        if ($appNotes) {
            $this->addUsingAlias(AppNotesPeer::NOTE_UID, $appNotes->getNoteUid(), Criteria::NOT_EQUAL);
        }
