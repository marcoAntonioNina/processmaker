...
abstract class BaseCalendarHolidays extends BaseObject  implements Persistent
{
	 ...
	 * Peer class name
	 */
	const PEER = 'CalendarHolidaysPeer';
	...
	{
		$this->calendar_uid = '';
		$this->calendar_holiday_name = '';
	}

	/**
	 * Initializes internal state of BaseCalendarHolidays object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [optionally formatted] temporal [calendar_holiday_start] column value.
	 * 
	 *
	...
	{
		if ($this->calendar_holiday_start === null) {
			return null;
		}


		if ($this->calendar_holiday_start === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->calendar_holiday_start);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->calendar_holiday_start, true), $x);
			}
		}
	...
	/**
	 * Get the [optionally formatted] temporal [calendar_holiday_end] column value.
	 * 
	 *
	 * @param      string $format The date/time format string (either date()-style or strftime()-style).
	 *							If format is NULL, then the raw DateTime object will be returned.
	...
	{
		if ($this->calendar_holiday_end === null) {
			return null;
		}


		if ($this->calendar_holiday_end === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->calendar_holiday_end);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->calendar_holiday_end, true), $x);
			}
		}
	 ...
	 * @return     CalendarHolidays The current object (for fluent API support)
	 */
	...
	public function setCalendarHolidayName($v)
	{
		...
		if ($this->calendar_holiday_name !== $v) {
	...
	/**
	 * Sets the value of [calendar_holiday_start] column to a normalized version of the date/time value specified.
	 * 
	...
	public function setCalendarHolidayStart($v)
	{
		...
		if ($this->calendar_holiday_start !== null || $dt !== null) {
			$currentDateAsString = ($this->calendar_holiday_start !== null && $tmpDt = new DateTime($this->calendar_holiday_start)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newDateAsString = $dt ? $dt->format('Y-m-d H:i:s') : null;
			if ($currentDateAsString !== $newDateAsString) {
				$this->calendar_holiday_start = $newDateAsString;
	...
	/**
	 * Sets the value of [calendar_holiday_end] column to a normalized version of the date/time value specified.
	 * 
	 * @param      mixed $v string, integer (timestamp), or DateTime value.
	 *               Empty strings are treated as NULL.
	 * @return     CalendarHolidays The current object (for fluent API support)
	...
	{
		$dt = PropelDateTime::newInstance($v, null, 'DateTime');
		if ($this->calendar_holiday_end !== null || $dt !== null) {
			$currentDateAsString = ($this->calendar_holiday_end !== null && $tmpDt = new DateTime($this->calendar_holiday_end)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newDateAsString = $dt ? $dt->format('Y-m-d H:i:s') : null;
			if ($currentDateAsString !== $newDateAsString) {
				$this->calendar_holiday_end = $newDateAsString;
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->calendar_holiday_name = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->calendar_holiday_start = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->calendar_holiday_end = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->resetModified();
			...
			return $startcol + 4; // 4 = CalendarHolidaysPeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = CalendarHolidaysPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = CalendarHolidaysQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				CalendarHolidaysPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	{
		if (isset($alreadyDumpedObjects['CalendarHolidays'][serialize($this->getPrimaryKey())])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['CalendarHolidays'][serialize($this->getPrimaryKey())] = true;
	...
	{
		return (null === $this->getCalendarUid()) && (null === $this->getCalendarHolidayName());
	}
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setCalendarHolidayName($this->getCalendarHolidayName());
		$copyObj->setCalendarHolidayStart($this->getCalendarHolidayStart());
		$copyObj->setCalendarHolidayEnd($this->getCalendarHolidayEnd());
		if ($makeNew) {
			$copyObj->setNew(true);
		}
	}
	...
	public function clear()
	{
		...
		$this->calendar_holiday_name = null;
		$this->calendar_holiday_start = null;
		$this->calendar_holiday_end = null;
		$this->alreadyInSave = false;
		$this->alreadyInValidation = false;
	...
	{
		return (string) $this->exportTo(CalendarHolidaysPeer::DEFAULT_STRING_FORMAT);
	}
