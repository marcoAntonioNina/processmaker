...
abstract class BaseAppSolrQueue extends BaseObject  implements Persistent
{
	...
	/**
	 * Initializes internal state of BaseAppSolrQueue object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     AppSolrQueue The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->app_updated = ($row[$startcol + 1] !== null) ? (int) $row[$startcol + 1] : null;
			$this->resetModified();
			...
			return $startcol + 2; // 2 = AppSolrQueuePeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = AppSolrQueuePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = AppSolrQueueQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				AppSolrQueuePeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	{
		if (isset($alreadyDumpedObjects['AppSolrQueue'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['AppSolrQueue'][$this->getPrimaryKey()] = true;
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setAppUpdated($this->getAppUpdated());
		if ($makeNew) {
			$copyObj->setNew(true);
		}
	}
	...
	{
		return (string) $this->exportTo(AppSolrQueuePeer::DEFAULT_STRING_FORMAT);
	}
