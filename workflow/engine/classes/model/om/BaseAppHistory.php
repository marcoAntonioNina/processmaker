...
abstract class BaseAppHistory extends BaseObject  implements Persistent
{
	...
	/**
	 * The value for the history_uid field.
	 * @var        int
	 */
	protected $history_uid;
	...
	/**
	 * Initializes internal state of BaseAppHistory object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [history_uid] column value.
	 * 
	 * @return     int
	 */
	public function getHistoryUid()
	{
		return $this->history_uid;
	}
	...
	/**
	 * Get the [optionally formatted] temporal [history_date] column value.
	 * 
	 *
	...
	public function getHistoryDate($format = 'Y-m-d H:i:s')
	{
		...
		if ($this->history_date === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->history_date);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->history_date, true), $x);
			}
		}
	...
	/**
	 * Set the value of [history_uid] column.
	 * 
	 * @param      int $v new value
	 * @return     AppHistory The current object (for fluent API support)
	 */
	public function setHistoryUid($v)
	{
		if ($v !== null) {
			$v = (int) $v;
		}

		if ($this->history_uid !== $v) {
			$this->history_uid = $v;
			$this->modifiedColumns[] = AppHistoryPeer::HISTORY_UID;
		}
	...
	/**
	 * Sets the value of [history_date] column to a normalized version of the date/time value specified.
	 * 
	...
	public function setHistoryDate($v)
	{
		...
		if ($this->history_date !== null || $dt !== null) {
			$currentDateAsString = ($this->history_date !== null && $tmpDt = new DateTime($this->history_date)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newDateAsString = $dt ? $dt->format('Y-m-d H:i:s') : null;
			if ($currentDateAsString !== $newDateAsString) {
				$this->history_date = $newDateAsString;
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->history_uid = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			$this->app_uid = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->del_index = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			$this->pro_uid = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->tas_uid = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->dyn_uid = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			$this->usr_uid = ($row[$startcol + 6] !== null) ? (string) $row[$startcol + 6] : null;
			$this->app_status = ($row[$startcol + 7] !== null) ? (string) $row[$startcol + 7] : null;
			$this->history_date = ($row[$startcol + 8] !== null) ? (string) $row[$startcol + 8] : null;
			$this->history_data = ($row[$startcol + 9] !== null) ? (string) $row[$startcol + 9] : null;
			$this->resetModified();
			...
			return $startcol + 10; // 10 = AppHistoryPeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = AppHistoryPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = AppHistoryQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				AppHistoryPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(AppHistoryPeer::HISTORY_UID) ) {
						throw new PropelException('Cannot insert a value for auto-increment primary key ('.AppHistoryPeer::HISTORY_UID.')');
					}

					$pk = BasePeer::doInsert($criteria, $con);
					$affectedRows = 1;
					$this->setHistoryUid($pk);  //[IMV] update autoincrement primary key
					$this->setNew(false);
				} else {
	...
	{
		if (isset($alreadyDumpedObjects['AppHistory'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['AppHistory'][$this->getPrimaryKey()] = true;
		$keys = AppHistoryPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getHistoryUid(),
			$keys[1] => $this->getAppUid(),
			$keys[2] => $this->getDelIndex(),
			$keys[3] => $this->getProUid(),
			$keys[4] => $this->getTasUid(),
			$keys[5] => $this->getDynUid(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setHistoryUid($arr[$keys[0]]);
		...
		if (array_key_exists($keys[5], $arr)) $this->setDynUid($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setUsrUid($arr[$keys[6]]);
		if (array_key_exists($keys[7], $arr)) $this->setAppStatus($arr[$keys[7]]);
		if (array_key_exists($keys[8], $arr)) $this->setHistoryDate($arr[$keys[8]]);
		if (array_key_exists($keys[9], $arr)) $this->setHistoryData($arr[$keys[9]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(AppHistoryPeer::HISTORY_UID)) $criteria->add(AppHistoryPeer::HISTORY_UID, $this->history_uid);
	...
	{
		$criteria = new Criteria(AppHistoryPeer::DATABASE_NAME);
		$criteria->add(AppHistoryPeer::HISTORY_UID, $this->history_uid);
	...
	/**
	 * Generic method to set the primary key (history_uid column).
	 *
	...
	{
		return null === $this->getHistoryUid();
	}
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setDynUid($this->getDynUid());
		$copyObj->setUsrUid($this->getUsrUid());
		$copyObj->setAppStatus($this->getAppStatus());
		$copyObj->setHistoryDate($this->getHistoryDate());
		$copyObj->setHistoryData($this->getHistoryData());
		if ($makeNew) {
			$copyObj->setNew(true);
			$copyObj->setHistoryUid(NULL); // this is a auto-increment column, so set to default value
		}
	}
	...
	{
		return (string) $this->exportTo(AppHistoryPeer::DEFAULT_STRING_FORMAT);
	}
