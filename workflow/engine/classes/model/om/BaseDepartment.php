...
abstract class BaseDepartment extends BaseObject  implements Persistent
{
	...
	public function hasOnlyDefaultValues()
	{
			...
			if ($this->dep_location !== 0) {
				return false;
			}

			if ($this->dep_status !== 'ACTIVE') {
				return false;
			}

			if ($this->dep_ref_code !== '') {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->dep_uid = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			$this->dep_parent = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->dep_manager = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->dep_location = ($row[$startcol + 3] !== null) ? (int) $row[$startcol + 3] : null;
			$this->dep_status = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->dep_ref_code = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			$this->dep_ldap_dn = ($row[$startcol + 6] !== null) ? (string) $row[$startcol + 6] : null;
			$this->resetModified();
			...
			return $startcol + 7; // 7 = DepartmentPeer::NUM_HYDRATE_COLUMNS.
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setDepParent($this->getDepParent());
		$copyObj->setDepManager($this->getDepManager());
		$copyObj->setDepLocation($this->getDepLocation());
		$copyObj->setDepStatus($this->getDepStatus());
		$copyObj->setDepRefCode($this->getDepRefCode());
		$copyObj->setDepLdapDn($this->getDepLdapDn());
		if ($makeNew) {
			$copyObj->setNew(true);
		}
	}
	...
	{
		$this->dep_uid = null;
		$this->dep_parent = null;
		$this->dep_manager = null;
		$this->dep_location = null;
		$this->dep_status = null;
		$this->dep_ref_code = null;
		$this->dep_ldap_dn = null;
		$this->alreadyInSave = false;
		$this->alreadyInValidation = false;
