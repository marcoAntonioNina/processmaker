...
abstract class BaseCaseTracker extends BaseObject  implements Persistent
{
	 ...
	 * @var        int
	 */
	protected $ct_derivation_history;
	...
	public function applyDefaultValues()
	{
		$this->pro_uid = '0';
		$this->ct_map_type = '0';
		$this->ct_derivation_history = 0;
		$this->ct_message_history = 0;
	}

	/**
	 * Initializes internal state of BaseCaseTracker object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     CaseTracker The current object (for fluent API support)
	 */
	...
	public function setCtDerivationHistory($v)
	{
		...
		if ($this->ct_derivation_history !== $v) {
	...
	public function hasOnlyDefaultValues()
	{
			...
			if ($this->ct_map_type !== '0') {
				return false;
			}

			if ($this->ct_derivation_history !== 0) {
				return false;
			}

			if ($this->ct_message_history !== 0) {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->ct_map_type = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->ct_derivation_history = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			$this->ct_message_history = ($row[$startcol + 3] !== null) ? (int) $row[$startcol + 3] : null;
			$this->resetModified();
			...
			return $startcol + 4; // 4 = CaseTrackerPeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = CaseTrackerPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = CaseTrackerQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				CaseTrackerPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	{
		if (isset($alreadyDumpedObjects['CaseTracker'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['CaseTracker'][$this->getPrimaryKey()] = true;
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setCtMapType($this->getCtMapType());
		$copyObj->setCtDerivationHistory($this->getCtDerivationHistory());
		$copyObj->setCtMessageHistory($this->getCtMessageHistory());
		if ($makeNew) {
			$copyObj->setNew(true);
		}
	}
	...
	{
		$this->pro_uid = null;
		$this->ct_map_type = null;
		$this->ct_derivation_history = null;
		$this->ct_message_history = null;
		$this->alreadyInSave = false;
		$this->alreadyInValidation = false;
	...
	{
		return (string) $this->exportTo(CaseTrackerPeer::DEFAULT_STRING_FORMAT);
	}
