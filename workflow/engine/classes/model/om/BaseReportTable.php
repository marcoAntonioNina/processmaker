...
abstract class BaseReportTable extends BaseObject  implements Persistent
{
	...
	public function applyDefaultValues()
	{
		...
		$this->rep_tab_type = '';
		$this->rep_tab_grid = '';
		$this->rep_tab_connection = '';
		$this->rep_tab_status = 'ACTIVE';
	}

	/**
	 * Initializes internal state of BaseReportTable object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [optionally formatted] temporal [rep_tab_create_date] column value.
	 * 
	 *
	...
	{
		if ($this->rep_tab_create_date === null) {
			return null;
		}


		if ($this->rep_tab_create_date === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->rep_tab_create_date);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->rep_tab_create_date, true), $x);
			}
		}
	 ...
	 * @return     ReportTable The current object (for fluent API support)
	 */
	...
	/**
	 * Sets the value of [rep_tab_create_date] column to a normalized version of the date/time value specified.
	 * 
	...
	public function setRepTabCreateDate($v)
	{
		...
		if ($this->rep_tab_create_date !== null || $dt !== null) {
			$currentDateAsString = ($this->rep_tab_create_date !== null && $tmpDt = new DateTime($this->rep_tab_create_date)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newDateAsString = $dt ? $dt->format('Y-m-d H:i:s') : null;
			if ($currentDateAsString !== $newDateAsString) {
				$this->rep_tab_create_date = $newDateAsString;
	...
	public function hasOnlyDefaultValues()
	{
			...
			if ($this->rep_tab_name !== '') {
				return false;
			}

			if ($this->rep_tab_type !== '') {
				return false;
			}

			if ($this->rep_tab_grid !== '') {
				return false;
			}

			if ($this->rep_tab_connection !== '') {
				return false;
			}

			if ($this->rep_tab_status !== 'ACTIVE') {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->rep_tab_uid = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			$this->pro_uid = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->rep_tab_name = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->rep_tab_type = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->rep_tab_grid = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->rep_tab_connection = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			$this->rep_tab_create_date = ($row[$startcol + 6] !== null) ? (string) $row[$startcol + 6] : null;
			$this->rep_tab_status = ($row[$startcol + 7] !== null) ? (string) $row[$startcol + 7] : null;
			$this->resetModified();
			...
			return $startcol + 8; // 8 = ReportTablePeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = ReportTablePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = ReportTableQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				ReportTablePeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	{
		if (isset($alreadyDumpedObjects['ReportTable'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['ReportTable'][$this->getPrimaryKey()] = true;
	...
	{
		$copyObj->setRepTabUid($this->getRepTabUid());
		$copyObj->setProUid($this->getProUid());
		$copyObj->setRepTabName($this->getRepTabName());
		$copyObj->setRepTabType($this->getRepTabType());
		$copyObj->setRepTabGrid($this->getRepTabGrid());
		$copyObj->setRepTabConnection($this->getRepTabConnection());
		$copyObj->setRepTabCreateDate($this->getRepTabCreateDate());
		$copyObj->setRepTabStatus($this->getRepTabStatus());
		if ($makeNew) {
			$copyObj->setNew(true);
		}
	}
	...
	{
		$this->rep_tab_uid = null;
		$this->pro_uid = null;
		$this->rep_tab_name = null;
		$this->rep_tab_type = null;
		$this->rep_tab_grid = null;
		$this->rep_tab_connection = null;
		$this->rep_tab_create_date = null;
		$this->rep_tab_status = null;
		$this->alreadyInSave = false;
		$this->alreadyInValidation = false;
	...
	{
		return (string) $this->exportTo(ReportTablePeer::DEFAULT_STRING_FORMAT);
	}
