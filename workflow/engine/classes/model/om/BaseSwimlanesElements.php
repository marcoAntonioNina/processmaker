...
abstract class BaseSwimlanesElements extends BaseObject  implements Persistent
{
	 ...
	 * Peer class name
	 */
	const PEER = 'SwimlanesElementsPeer';
	 ...
	 * Note: this column has a database default value of: 'LINE'
	 * @var        string
	 */
	protected $swi_type;
	...
	/**
	 * Initializes internal state of BaseSwimlanesElements object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     SwimlanesElements The current object (for fluent API support)
	 */
	...
	public function hasOnlyDefaultValues()
	{
			...
			if ($this->swi_height !== 0) {
				return false;
			}

			if ($this->swi_next_uid !== '') {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->swi_uid = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			$this->pro_uid = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->swi_type = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->swi_x = ($row[$startcol + 3] !== null) ? (int) $row[$startcol + 3] : null;
			$this->swi_y = ($row[$startcol + 4] !== null) ? (int) $row[$startcol + 4] : null;
			$this->swi_width = ($row[$startcol + 5] !== null) ? (int) $row[$startcol + 5] : null;
			$this->swi_height = ($row[$startcol + 6] !== null) ? (int) $row[$startcol + 6] : null;
			$this->swi_next_uid = ($row[$startcol + 7] !== null) ? (string) $row[$startcol + 7] : null;
			$this->resetModified();
			...
			return $startcol + 8; // 8 = SwimlanesElementsPeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = SwimlanesElementsPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = SwimlanesElementsQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				SwimlanesElementsPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	{
		if (isset($alreadyDumpedObjects['SwimlanesElements'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['SwimlanesElements'][$this->getPrimaryKey()] = true;
	...
	{
		$copyObj->setSwiUid($this->getSwiUid());
		$copyObj->setProUid($this->getProUid());
		$copyObj->setSwiType($this->getSwiType());
		$copyObj->setSwiX($this->getSwiX());
		$copyObj->setSwiY($this->getSwiY());
		$copyObj->setSwiWidth($this->getSwiWidth());
		$copyObj->setSwiHeight($this->getSwiHeight());
		$copyObj->setSwiNextUid($this->getSwiNextUid());
		if ($makeNew) {
			$copyObj->setNew(true);
		}
	}
	...
	public function clear()
	{
		...
		$this->swi_y = null;
		$this->swi_width = null;
		$this->swi_height = null;
		$this->swi_next_uid = null;
		$this->alreadyInSave = false;
		$this->alreadyInValidation = false;
	...
	{
		return (string) $this->exportTo(SwimlanesElementsPeer::DEFAULT_STRING_FORMAT);
	}
