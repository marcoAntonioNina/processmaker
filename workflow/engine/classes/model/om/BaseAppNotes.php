...
abstract class BaseAppNotes extends BaseObject  implements Persistent
{
	...
	/**
	 * The value for the notes_uid field.
	 * @var        int
	 */
	protected $notes_uid;
	...
	/**
	 * Get the [notes_uid] column value.
	 * 
	 * @return     int
	 */
	public function getNotesUid()
	{
		return $this->notes_uid;
	}
	...
	/**
	 * Set the value of [notes_uid] column.
	 * 
	 * @param      int $v new value
	 * @return     AppNotes The current object (for fluent API support)
	 */
	public function setNotesUid($v)
	{
		if ($v !== null) {
			$v = (int) $v;
		}
		...
		if ($this->notes_uid !== $v) {
			$this->notes_uid = $v;
			$this->modifiedColumns[] = AppNotesPeer::NOTES_UID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->notes_uid = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(AppNotesPeer::NOTES_UID) ) {
						throw new PropelException('Cannot insert a value for auto-increment primary key ('.AppNotesPeer::NOTES_UID.')');
					}

					$pk = BasePeer::doInsert($criteria, $con);
					$affectedRows = 1;
					$this->setNotesUid($pk);  //[IMV] update autoincrement primary key
					$this->setNew(false);
				} else {
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setNotesUid($arr[$keys[0]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(AppNotesPeer::NOTES_UID)) $criteria->add(AppNotesPeer::NOTES_UID, $this->notes_uid);
	...
	{
		$criteria = new Criteria(AppNotesPeer::DATABASE_NAME);
		$criteria->add(AppNotesPeer::NOTES_UID, $this->notes_uid);
	...
	/**
	 * Generic method to set the primary key (notes_uid column).
	 *
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		if ($makeNew) {
			$copyObj->setNew(true);
			$copyObj->setNotesUid(NULL); // this is a auto-increment column, so set to default value
		}
	}
