...
abstract class BaseContent extends BaseObject  implements Persistent
{
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->con_category = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			$this->con_parent = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->con_id = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->con_lang = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->con_value = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->resetModified();
	...
	{
		if (isset($alreadyDumpedObjects['Content'][serialize($this->getPrimaryKey())])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Content'][serialize($this->getPrimaryKey())] = true;
	...
	{
		return (null === $this->getConCategory()) && (null === $this->getConParent()) && (null === $this->getConId()) && (null === $this->getConLang());
	}
	...
	{
		$copyObj->setConCategory($this->getConCategory());
		$copyObj->setConParent($this->getConParent());
		$copyObj->setConId($this->getConId());
		$copyObj->setConLang($this->getConLang());
		$copyObj->setConValue($this->getConValue());
		if ($makeNew) {
			$copyObj->setNew(true);
		}
	}
