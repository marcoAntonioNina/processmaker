...
abstract class BaseShadowTable extends BaseObject  implements Persistent
{
	...
	/**
	 * Initializes internal state of BaseShadowTable object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [optionally formatted] temporal [shd_date] column value.
	 * 
	 *
	...
	{
		if ($this->shd_date === null) {
			return null;
		}


		if ($this->shd_date === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->shd_date);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->shd_date, true), $x);
			}
		}
	 ...
	 * @return     ShadowTable The current object (for fluent API support)
	 */
	...
	/**
	 * Sets the value of [shd_date] column to a normalized version of the date/time value specified.
	 * 
	...
	public function setShdDate($v)
	{
		...
		if ($this->shd_date !== null || $dt !== null) {
			$currentDateAsString = ($this->shd_date !== null && $tmpDt = new DateTime($this->shd_date)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newDateAsString = $dt ? $dt->format('Y-m-d H:i:s') : null;
			if ($currentDateAsString !== $newDateAsString) {
				$this->shd_date = $newDateAsString;
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->shd_uid = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			$this->add_tab_uid = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->shd_action = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->shd_details = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->usr_uid = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->app_uid = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			$this->shd_date = ($row[$startcol + 6] !== null) ? (string) $row[$startcol + 6] : null;
			$this->resetModified();
			...
			return $startcol + 7; // 7 = ShadowTablePeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = ShadowTablePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = ShadowTableQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				ShadowTablePeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	{
		if (isset($alreadyDumpedObjects['ShadowTable'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['ShadowTable'][$this->getPrimaryKey()] = true;
	...
	{
		$copyObj->setShdUid($this->getShdUid());
		$copyObj->setAddTabUid($this->getAddTabUid());
		$copyObj->setShdAction($this->getShdAction());
		$copyObj->setShdDetails($this->getShdDetails());
		$copyObj->setUsrUid($this->getUsrUid());
		$copyObj->setAppUid($this->getAppUid());
		$copyObj->setShdDate($this->getShdDate());
		if ($makeNew) {
			$copyObj->setNew(true);
		}
	}
	...
	public function clear()
	{
		...
		$this->shd_action = null;
		$this->shd_details = null;
		$this->usr_uid = null;
		$this->app_uid = null;
		$this->shd_date = null;
		$this->alreadyInSave = false;
	...
	{
		return (string) $this->exportTo(ShadowTablePeer::DEFAULT_STRING_FORMAT);
	}
