...
abstract class BaseStepSupervisor extends BaseObject  implements Persistent
{
	...
	public function applyDefaultValues()
	{
		$this->step_uid = '';
		$this->pro_uid = '0';
		$this->step_type_obj = 'DYNAFORM';
		$this->step_uid_obj = '0';
		$this->step_position = 0;
	}

	/**
	 * Initializes internal state of BaseStepSupervisor object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     StepSupervisor The current object (for fluent API support)
	 */
	...
	public function hasOnlyDefaultValues()
	{
			...
			if ($this->step_type_obj !== 'DYNAFORM') {
				return false;
			}

			if ($this->step_uid_obj !== '0') {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->step_uid = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			$this->pro_uid = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->step_type_obj = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->step_uid_obj = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->step_position = ($row[$startcol + 4] !== null) ? (int) $row[$startcol + 4] : null;
			$this->resetModified();
			...
			return $startcol + 5; // 5 = StepSupervisorPeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = StepSupervisorPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = StepSupervisorQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				StepSupervisorPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	{
		if (isset($alreadyDumpedObjects['StepSupervisor'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['StepSupervisor'][$this->getPrimaryKey()] = true;
	...
	{
		$copyObj->setStepUid($this->getStepUid());
		$copyObj->setProUid($this->getProUid());
		$copyObj->setStepTypeObj($this->getStepTypeObj());
		$copyObj->setStepUidObj($this->getStepUidObj());
		$copyObj->setStepPosition($this->getStepPosition());
		if ($makeNew) {
			$copyObj->setNew(true);
		}
	}
	...
	public function clear()
	{
		$this->step_uid = null;
		$this->pro_uid = null;
		$this->step_type_obj = null;
		$this->step_uid_obj = null;
		$this->step_position = null;
		$this->alreadyInSave = false;
		$this->alreadyInValidation = false;
	...
	{
		return (string) $this->exportTo(StepSupervisorPeer::DEFAULT_STRING_FORMAT);
	}
