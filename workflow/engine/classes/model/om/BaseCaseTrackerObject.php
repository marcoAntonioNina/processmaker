...
abstract class BaseCaseTrackerObject extends BaseObject  implements Persistent
{
	 ...
	 * Peer class name
	 */
	const PEER = 'CaseTrackerObjectPeer';
	 ...
	 * Note: this column has a database default value of: 'DYNAFORM'
	 * @var        string
	 */
	protected $cto_type_obj;
	...
	public function applyDefaultValues()
	{
		$this->cto_uid = '';
		$this->pro_uid = '0';
		$this->cto_type_obj = 'DYNAFORM';
		$this->cto_uid_obj = '0';
		$this->cto_position = 0;
	}

	/**
	 * Initializes internal state of BaseCaseTrackerObject object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     CaseTrackerObject The current object (for fluent API support)
	 */
	...
	public function hasOnlyDefaultValues()
	{
			...
			if ($this->cto_type_obj !== 'DYNAFORM') {
				return false;
			}

			if ($this->cto_uid_obj !== '0') {
				return false;
			}

			if ($this->cto_position !== 0) {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->cto_uid = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			$this->pro_uid = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->cto_type_obj = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->cto_uid_obj = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->cto_condition = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->cto_position = ($row[$startcol + 5] !== null) ? (int) $row[$startcol + 5] : null;
			$this->resetModified();
			...
			return $startcol + 6; // 6 = CaseTrackerObjectPeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = CaseTrackerObjectPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = CaseTrackerObjectQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				CaseTrackerObjectPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	{
		if (isset($alreadyDumpedObjects['CaseTrackerObject'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['CaseTrackerObject'][$this->getPrimaryKey()] = true;
	...
	{
		$copyObj->setCtoUid($this->getCtoUid());
		$copyObj->setProUid($this->getProUid());
		$copyObj->setCtoTypeObj($this->getCtoTypeObj());
		$copyObj->setCtoUidObj($this->getCtoUidObj());
		$copyObj->setCtoCondition($this->getCtoCondition());
		$copyObj->setCtoPosition($this->getCtoPosition());
		if ($makeNew) {
			$copyObj->setNew(true);
		}
	}
	...
	public function clear()
	{
		$this->cto_uid = null;
		$this->pro_uid = null;
		$this->cto_type_obj = null;
		$this->cto_uid_obj = null;
		$this->cto_condition = null;
		$this->cto_position = null;
		$this->alreadyInSave = false;
		$this->alreadyInValidation = false;
	...
	{
		return (string) $this->exportTo(CaseTrackerObjectPeer::DEFAULT_STRING_FORMAT);
	}
