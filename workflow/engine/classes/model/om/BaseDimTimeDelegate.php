...
abstract class BaseDimTimeDelegate extends BaseObject  implements Persistent
{
	 ...
	 * Peer class name
	 */
	const PEER = 'DimTimeDelegatePeer';
	...
	/**
	 * Initializes internal state of BaseDimTimeDelegate object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     DimTimeDelegate The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->time_id = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			$this->month_id = ($row[$startcol + 1] !== null) ? (int) $row[$startcol + 1] : null;
			$this->qtr_id = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			$this->year_id = ($row[$startcol + 3] !== null) ? (int) $row[$startcol + 3] : null;
			$this->month_name = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->month_desc = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			$this->qtr_name = ($row[$startcol + 6] !== null) ? (string) $row[$startcol + 6] : null;
			$this->qtr_desc = ($row[$startcol + 7] !== null) ? (string) $row[$startcol + 7] : null;
			$this->resetModified();
			...
			return $startcol + 8; // 8 = DimTimeDelegatePeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = DimTimeDelegatePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = DimTimeDelegateQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				DimTimeDelegatePeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	{
		if (isset($alreadyDumpedObjects['DimTimeDelegate'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['DimTimeDelegate'][$this->getPrimaryKey()] = true;
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setQtrId($this->getQtrId());
		$copyObj->setYearId($this->getYearId());
		$copyObj->setMonthName($this->getMonthName());
		$copyObj->setMonthDesc($this->getMonthDesc());
		$copyObj->setQtrName($this->getQtrName());
		$copyObj->setQtrDesc($this->getQtrDesc());
		if ($makeNew) {
			$copyObj->setNew(true);
		}
	}
	...
	{
		return (string) $this->exportTo(DimTimeDelegatePeer::DEFAULT_STRING_FORMAT);
	}
