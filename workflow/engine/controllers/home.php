...
class Home extends Controller
{
    ...
    public function indexSingle($httpData)
    {
        ...
        $steps    = $apps->getSteps($lastApp['APP_UID'], $lastApp['DEL_INDEX'], $lastApp['TAS_UID'],
            $lastApp['PRO_UID']);
        ...
        $this->setVar('default_url', "cases/cases_Open?APP_UID={$lastApp['APP_UID']}&DEL_INDEX=
            {$lastApp['DEL_INDEX']}&action=todo");
    ...
    public function startCase($httpData)
    {
        ...
        $oCase = new Cases();
        $aNextStep = $oCase->getNextStep($_SESSION['PROCESS'], $_SESSION['APPLICATION'], $_SESSION['INDEX'],
            $_SESSION['STEP_POSITION']);
        //../cases/cases_Open?APP_UID={$APP.APP_UID}&DEL_INDEX={$APP.DEL_INDEX}&action=todo
        $aNextStep['PAGE'] = '../cases/cases_Open?APP_UID='.$aData['APPLICATION'].'&DEL_INDEX='.$aData['INDEX'].
            '&action=draft';
