...
class SkinEngine
{
    ...
    private function _init()
    {
        ...
        if (strtolower($this->mainSkin) != "classic") {
            ...
            } elseif (is_dir($this->skinsBasePath . $this->mainSkin)) {
    ...
    private function _blank()
    {
        ...
        $smarty->assign('username', (isset($_SESSION['USR_USERNAME']) ?
            '(' . $_SESSION['USR_USERNAME'] . ' ' . G::LoadTranslation('ID_IN') . ' ' . SYS_SYS . ')' : ''));
        $smarty->assign('header', $header);
        $smarty->force_compile = $this->forceTemplateCompile;
    ...
    private function _submenu()
    {
        ...
        if (isset($G_ENABLE_BLANK_SKIN) && $G_ENABLE_BLANK_SKIN) {
            ...
            if (isset($oHeadPublisher)) {
                $oHeadPublisher->title = isset($_SESSION['USR_USERNAME']) ? '(' . $_SESSION['USR_USERNAME'] . ' ' .
                G::LoadTranslation('ID_IN') . ' ' . SYS_SYS . ')' : '';
                $header = $oHeadPublisher->printHeader();
                $header .= $oHeadPublisher->getExtJsStylesheets($this->cssFileName);
            }
            $footer = '';
            if (strpos($_SERVER['REQUEST_URI'], '/login/login') !== false) {
                if (DB_SYSTEM_INFORMATION == 1) {
                    $footer = "<a href=\"#\" onclick=\"openInfoPanel();return false;\" class=\"FooterLink\">| " .
                    G::LoadTranslation('ID_SYSTEM_INFO') . " |</a><br />";
                }
                $freeOfChargeText = "";
                if (! defined('SKIP_FREE_OF_CHARGE_TEXT')) {
                    $freeOfChargeText = "Supplied free of charge with no support, certification, warranty,
                    <br>maintenance nor indemnity by Colosa and its Certified Partners.";
                }
                $footer .= "<br />Copyright &copy; 2003-" . date('Y') . " <a href=\"http://www.colosa.com\"
                alt=\"Colosa, Inc.\" target=\"_blank\">Colosa, Inc.</a> All rights reserved.
                <br /> $freeOfChargeText " . "<br><br/><a href=\"http://www.processmaker.com\" alt=\"Powered by
                ProcessMaker - Open Source Workflow & Business Process Management (BPM) Management Software\"
                title=\"Powered by ProcessMaker\" target=\"_blank\"><img src=\"/images/PowerdbyProcessMaker.png\"
                border=\"0\" /></a>";
            }
            ...
            if (NO_DISPLAY_USERNAME == 0) {
                ...
                $uws = (isset($_SESSION['USR_ROLENAME']) && $_SESSION['USR_ROLENAME'] != '')?
                    strtolower(G::LoadTranslation('ID_WORKSPACE_USING')): G::LoadTranslation('ID_WORKSPACE_USING');
    ...
    private function _tracker()
    {
        ...
        if (isset($G_ENABLE_BLANK_SKIN) && $G_ENABLE_BLANK_SKIN) {
            ...
            $oMenu = new Menu();
            $menus = $oMenu->generateArrayForTemplate ($G_MAIN_MENU,'SelectedMenu', 'mainMenu',
                $G_MENU_SELECTED, $G_ID_MENU_SELECTED);
            $smarty->assign('menus', $menus );
    ...
    private function _default()
    {
        ...
        if (isset($G_ENABLE_BLANK_SKIN) && $G_ENABLE_BLANK_SKIN) {
            ...
            if (strpos($_SERVER['REQUEST_URI'], '/login/login') !== false) {
                ...
                $footer .= "<br />Copyright &copy; 2003-" . date('Y') . " <a href=\"http://www.colosa.com\"
                alt=\"Colosa, Inc.\" target=\"_blank\">Colosa, Inc.</a> All rights reserved.<br />
                $freeOfChargeText " . "<br><br/><a href=\"http://www.processmaker.com\" alt=\"Powered by
                ProcessMaker - Open Source Workflow & Business Process Management (BPM) Management Software\"
                title=\"Powered by ProcessMaker\" target=\"_blank\"><img src=\"/images/PowerdbyProcessMaker.png\"
                border=\"0\" /></a>";
            }
            ...
            if (class_exists('PMPluginRegistry')) {
                ...
                if (isset($sFotoSelect) && $sFotoSelect!='' && !(strcmp($sWspaceSelect, SYS_SYS))) {
                    ...
                    $sCompanyLogo = "/sys".SYS_SYS."/".SYS_LANG."/".SYS_SKIN."/setup/showLogoFile.php?id=".
