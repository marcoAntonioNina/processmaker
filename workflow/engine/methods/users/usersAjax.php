...
switch($_POST['action'])
{
            ...
            if ($form['USR_UID'] == '') {
                ...
                if ($_FILES['USR_PHOTO']['error'] != 1) {
                    if ($_FILES['USR_PHOTO']['tmp_name'] != '') {
                        G::uploadFile($_FILES['USR_PHOTO']['tmp_name'], PATH_IMAGES_ENVIRONMENT_USERS, $sUserUID
                                      . '.gif');
                    }
                } else {
                ...
                if (isset($form['USR_PASSWORD'])) {
                    if ($form['USR_PASSWORD'] != '') {
                        ...
                        $aUserProperty = $oUserProperty->loadOrCreateIfNotExists($form['USR_UID'],
                            array('USR_PASSWORD_HISTORY' => serialize(array(md5($form['USR_PASSWORD'])))));
                        ...
                        $aErrors = $oUserProperty->validatePassword($form['USR_NEW_PASS'],
                            $aUserProperty['USR_LAST_UPDATE_DATE'], 0);
                        if (count($aErrors) > 0) {
                            ...
                            foreach ($aErrors as $sError) {
                                switch ($sError) {
                                    ...
                                    case 'ID_PPP_EXPIRATION_IN':
                                        $sDescription .= ' - ' . G::LoadTranslation($sError).' ' .
                                        PPP_EXPIRATION_IN . ' ' . G::LoadTranslation('ID_DAYS') . ',';
                                        break;
                                    default:
                                        $sDescription .= ' - ' . G::LoadTranslation($sError).',';
                                        break;
                        ...
                        if (PPP_PASSWORD_HISTORY > 0) {
                            ...
                            if ($sw == 0) {
                                ...
                                $sDescription .= ' - ' . G::LoadTranslation('PASSWORD_HISTORY').': ' .
                                PPP_PASSWORD_HISTORY . '<br />';
                ...
                if ($_FILES['USR_PHOTO']['error'] != 1) {
                    if ($_FILES['USR_PHOTO']['tmp_name'] != '') {
                        ...
                        G::uploadFile($_FILES['USR_PHOTO']['tmp_name'], PATH_IMAGES_ENVIRONMENT_USERS,
                                        $aData['USR_UID'] . '.' . $aAux[1]);
                        G::resizeImage(PATH_IMAGES_ENVIRONMENT_USERS . $aData['USR_UID'] . '.' . $aAux[1], 96,
                                       96, PATH_IMAGES_ENVIRONMENT_USERS . $aData['USR_UID'] . '.gif');
                    }
                } else {
                    $result->success   = false;
                    $result->fileError = true;
    ...
    case 'userData':
        ...
        $aFields ['USR_CALENDAR'] = $calendarInfo ['CALENDAR_APPLIED'] != 'DEFAULT' ?
        $calendarInfo ['CALENDAR_UID']: "";
    ...
    case 'defaultMainMenuOptionList';
        foreach ($RBAC->aUserInfo['PROCESSMAKER']['PERMISSIONS'] as $permission) {
            switch($permission['PER_CODE']){
                //Permission is checked of user's
                case 'PM_USERS':
                case 'PM_SETUP':
