...
class G
{
     ...
     * @return Boolean
     */
    public function var_compare($value=true, $varN)
    {
        $nums = func_num_args();
        if ($nums<2) {
            return true;
    ...
    public function getMacAddress()
    {
        if (strstr (getenv ('OS'), 'Windows')) {
            ...
            preg_match('/[\dA-Z]{2,2}[\:-][\dA-Z]{2,2}[\:-][\dA-Z]{2,2}[\:-][\dA-Z]{2,2}[\:-][\dA-Z]{2,2}[\:-]'.
            '[\dA-Z]{2,2}/i', $ipconfig,$mac);
        } else {
            $ifconfig = `/sbin/ifconfig`;
            preg_match('/[\dA-Z]{2,2}[\:-][\dA-Z]{2,2}[\:-][\dA-Z]{2,2}[\:-][\dA-Z]{2,2}[\:-][\dA-Z]{2,2}[\:-]'.
            '[\dA-Z]{2,2}/i',$ifconfig,$mac);
        }
    ...
    {
        // The ereg public function has been DEPRECATED as of PHP 5.3.0.
        ...
        if (preg_match("/(error</b>:)(.+)(<br)/", $buffer, $regs)) {
            ...
            $errorBox = "<table cellpadding=1 cellspacing=0 border=0 bgcolor=#808080 width=250><tr><td >" .
                        ...
                        "<tr bgcolor=#d04040><td colspan=2 nowrap><font color=#ffffaa><code> ERROR CAUGHT check log".
                        "file</code></font></td></tr>" .
                        "<tr ><td colspan=2 nowrap><font color=black><code>IP address: $ip_addr</code></font></td>".
                        "</tr> " .
                        "</table></td></tr></table>";
            return $errorBox;
        }
    ...
    /*** Encrypt and decrypt public functions ****/
    /**
     * Encrypt string
     *
     * @author Fernando Ontiveros Lira <fernando@colosa.com>
     ...
     * @return string
     */
    static public function parseURI($uri, $config=array())
    {
    ...
    /** 
     * public function to calculate the time used to render a page  
     */
    public function logTimeByPage()
    {
        ...
        fwrite($fpt, sprintf ("%s.%03d %15s %s %5.3f %s\n", date('Y-m-d H:i:s'), $time, getenv('REMOTE_ADDR'),
            substr($serverAddr,-4), $time, $_SERVER['REQUEST_URI']));
        fclose($fpt);
    }
    /**
    ...
    public function streamCSSBigFile($filename)
    {
        ...
        //G::pr($xmlConfigurationObj->result['skinConfiguration']['__CONTENT__']['cssFiles']['__CONTENT__']
        //[$tempSkin]['__CONTENT__']  );
        $skinFilesArray=$xmlConfigurationObj->result['skinConfiguration']['__CONTENT__']['cssFiles']
        ['__CONTENT__'][$skinVariant]['__CONTENT__']['cssFile'] ;
        //G::pr($skinFilesArray);
        foreach ($skinFilesArray as $keyFile => $cssFileInfo) {
            ...
            if (((in_array($browserName, $enabledBrowsers))||(in_array('ALL', $enabledBrowsers)))
                &&(!(in_array($browserName, $disabledBrowsers)))) {
                //G::pr($cssFileInfo['__ATTRIBUTES__']['file']);
                $output .= file_get_contents ($baseSkinDirectory . PATH_SEP.'css'.PATH_SEP.
                    $cssFileInfo['__ATTRIBUTES__']['file']);
            }
        }
        //Remove comments..
        $regex = array(
                ...
                switch ($jsName) {
                    ...
                    case 'ext-all.js':
                        ...
                        if (is_file($cfStored['filename']) && $checksum == $cfStored['checksum']) {
                            ...
                            $output .= JSMin::minify (file_get_contents ($pathJs .
                                'ext/ux.locationbar/Ext.ux.LocationBar.js'));
                            $output .= JSMin::minify (file_get_contents ($pathJs .
                                'ext/ux.statusbar/ext-statusbar.js'));
                            $output .= JSMin::minify (file_get_contents ($pathJs .
                                'ext/ux.treefilterx/Ext.ux.tree.TreeFilterX.js'));
    ...
    public function formatDate($datetime, $format='Y-m-d', $lang='')
    {
        ...
        // Spanish months
        $ARR_MONTHS['es'] = array ("Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto",
                                   "Septiembre","Octubre","Noviembre","Diciembre");
        // English months
        $ARR_MONTHS['en'] = array("January", "February", "March", "April", "May", "June","July", "August",
    ...
    public function arrayDiff($array1, $array2)
    {
        ...
        // this is necessary because the array_diff public function returns arrays that retain their original keys
     ...
     *     @fn()  Evaluate string with the public function "fn"
     * @author David Callizaya <calidavidx21@hotmail.com>
     */
    public function replaceDataField($sqlString, $result, $DBEngine='mysql')
    {
        ...
        //(?:[\\\\][\w\W])?)*)\))/',$sqlString,$match,PREG_PATTERN_ORDER | PREG_OFFSET_CAPTURE);
        $count = preg_match_all('/\@(?:([\@\%\#\=\!Qq])([a-zA-Z\_]\w*)|([a-zA-Z\_][\w\-\>\:]*)
            \(((?:[^\\\\\)]*?)*)\))/',$sqlString,$match,PREG_PATTERN_ORDER | PREG_OFFSET_CAPTURE);
        if ($count) {
            for ($r=0; $r<$count; $r++) {
                ...
                if (!is_array($result[$match[2][$r][0]])) {
                    ...
                    if (($match[1][$r][0]==='')&&($match[2][$r][0]==='')&&($match[3][$r][0]!=='')) {
                        eval('$strAux = ' . $match[3][$r][0] . '(\'' . addcslashes(
                            G::replaceDataField(stripslashes($match[4][$r][0]),$result),'\\\'') . '\');');
    ...
    /**
     * public function LoadMessageXml
     * @author David S. Callizaya S. <davidsantos@colosa.com>
     * @access public
     * @parameter string msgID
     * @parameter string file
    ...
    /**
     * public function LoadTranslationObject
     ...
     * @parameter string msgID
     * @parameter string file
     * @parameter array data // erik: associative array within data input to replace for formatted string i.e
     * "any messsage {replaced_label} that contains a replace label"
     * @return string
     */
    ...
    public function http_build_query($formdata, $numeric_prefix=null, $key=null)
    {
        ...
        foreach ((array)$formdata as $k => $v) {
            ...
            if (is_array($v) || is_object($v)) {
                $res[] = G::http_build_query($v,
                    null /* or $numeric_prefix if you want to add numeric_prefix to all indexes in array*/, $tmp_key);
            } else {
                $res[] = $tmp_key."=".rawurlencode($v);
            }
            /*
             If you want, you can write this as one string:
             $res[] = ((is_array($v) || is_object($v)) ? G::http_build_query($v, null,
             $tmp_key) : $tmp_key."=".urlencode($v));
             */
        }
    ...
    public function resizeImage($path, $resWidth, $resHeight, $saveTo=null)
    {
        ...
        $memoryNeeded = Round(($imageInfo[0] * $imageInfo[1] * $imageInfo['bits'] * $imageInfo['channels'] +
            Pow(2, 16)) * 1.95) / (1024*1024);
        if ($memoryNeeded < 80) {
            $memoryNeeded = 80;
        }
    ...
    public function getSystemConstants($params=null)
    {
        ...
        //###############################################################################################
        // Added for compatibility betweek aplication called from web Entry that uses just WS public functions
        //###############################################################################################
    ...
    /*************************************** init **********************************************
     * Xml parse collection public functions
    ...
    /**
     *  Inflects a string with accented characters and other characteres not suitable for file names, by defaul
     *  replace with undescore
     *
     ...
     * Returns an array when has a valid reponse, if the response is invalid returns an object containing a
     * status_code and message properties.
     *
     * @author <erik@colosa.com>
     */
     ...
     *      -> returns a object within $o->email => erik@colosa.com and $o->name => erik A.O. in other case returns
     *      false
     *
     */
     ...
     * this information is retrieved by the PMpublic function getEmailConfiguration()
     *  
     * @author Erik Amaru Ortiz <erik@colosa.com>
     ...
     * @return mixed boolean or string : if the email was sent successfully returns true, otherwise returns a
     * string within error message
     */
    public function sendMail($from, $fromName, $address, $subject, $body)
    {
        // require_once "classes/class.pmpublic functions.php";
        G::LoadClass("pmpublic functions");
     ...
     */
    public public function gettype($var)
    {
        switch ($var) {
            case is_null($var):
                $type='NULL';
    ...
    /**
     * Recursive Is writeable public function
     *
     * @author Erik Amaru Ortiz <erik@colosa.com>
     *
    ...
    /**
     * Recursive version of glob php standard public function
     *
     * @author Erik Amaru Ortiz <erik@colosa.com>
     *
     * @param $path path to scan recursively the write permission
     * @param $flags to notive glob public function
     * @param $pattern pattern to filter some especified files
    ...
    public function browser_detection($which_test, $test_excludes='', $external_ua_string='')
    {
        ...
        $b_safe_browser, $browser_name, $browser_number, $browser_math_number, $browser_user_agent,
        $browser_working, $ie_version, $mobile_test, $moz_number, $moz_rv, $moz_rv_full, $moz_release_date,
        $moz_type, $os_number, $os_type, $true_ie_number, $ua_type, $webkit_type, $webkit_type_number;
        */
        ...
        */
        if (!$b_repeat) {
            ...
            */
            $a_moz_types = array('bonecho', 'camino', 'epiphany', 'firebird', 'flock', 'galeon', 'iceape', 'icecat',
                                 'k-meleon', 'minimo', 'multizilla', 'phoenix', 'songbird', 'swiftfox', 'seamonkey',
                                 'shiretoko', 'iceweasel', 'firefox', 'minefield', 'netscape6', 'netscape', 'rv');
            ...
            */
            $a_webkit_types = array('arora', 'chrome', 'epiphany', 'gtklauncher', 'konqueror', 'midori', 'omniweb',
                                    'safari', 'uzbl', 'applewebkit', 'webkit');
                    ...
                    switch ($browser_working) {
                        ...
                        case 'moz':
                            ...
                            */
                            ...
                            // gets the actual release date, necessary if you need to do public functionality tests
                        ...
                        case 'ie':
                            ...
                            */
                            $browser_number = G::get_item_version($browser_user_agent, $browser_name, true,
                                'trident/');
        ...
        /*
        This is where you return values based on what parameter you used to call the public function
    ...
    public function get_os_data ($pv_browser_string, $pv_browser_name, $pv_version_number)
    {
        ...
        */
        ...
        $a_unix_types = array('dragonfly', 'freebsd', 'openbsd', 'netbsd', 'bsd', 'unixware', 'solaris', 'sunos',
            'sun4', 'sun5', 'suni86', 'sun', 'irix5', 'irix6', 'irix', 'hpux9', 'hpux10', 'hpux11',
            'hpux', 'hp-ux', 'aix1', 'aix2', 'aix3', 'aix4', 'aix5', 'aix', 'sco', 'unixware',
            'mpras', 'reliant', 'dec', 'sinix', 'unix');
        ...
        $a_linux_distros = array('ubuntu', 'kubuntu', 'xubuntu', 'mepis', 'xandros', 'linspire', 'winspire',
            'jolicloud', 'sidux', 'kanotix', 'debian', 'opensuse', 'suse', 'fedora', 'redhat',
            'slackware', 'slax', 'mandrake', 'mandriva', 'gentoo', 'sabayon', 'linux');
        $a_linux_process = array ('i386', 'i586', 'i686');// not use currently
        // note, order of os very important in os array, you will get failed ids if changed
        $a_os_types = array('android', 'blackberry', 'iphone', 'palmos', 'palmsource', 'symbian', 'beos', 'os2',
            'amiga', 'webtv', 'mac', 'nt', 'win', $a_unix_types, $a_linux_distros);
        ...
        for ($i = 0; $i < $i_count; $i++) {
            ...
            */
            if (!is_array($os_working_data) && strstr($pv_browser_string, $os_working_data)
                    ...
                    case 'mac':
                        if (strstr($pv_browser_string, 'os x')) {
                            ...
                            if (strstr($pv_browser_string, 'os x ')) {
                                ...
                                $os_working_number = str_replace('_', '.', G::get_item_version($pv_browser_string,
                                    'os x'));
                            } else {
        ...
        // pack the os data array for return to main public function
        ...
        */
        $a_mobile_search = array(
            ...
            */
            ...
            'benq', 'blackberry', 'danger hiptop', 'ddipocket', ' droid', 'ipad', 'ipod', 'iphone', 'kindle', 'lge-cx',
            'lge-lx', 'lge-mx', 'lge vx', 'lge ', 'lge-', 'lg;lx', 'nintendo wii', 'nokia', 'palm', 'pdxgw',
            'playstation', 'sagem', 'samsung', 'sec-sgh', 'sharp', 'sonyericsson', 'sprint', 'zune', 'j-phone', 'n410',
            'mot 24', 'mot-', 'htc-', 'htc_', 'htc ', 'sec-', 'sie-m', 'sie-s', 'spv ', 'vodaphone', 'smartphone',
            'armv', 'midp', 'mobilephone',
            // browsers
            'avantgo', 'blazer', 'elaine', 'eudoraweb', 'iemobile',  'minimo', 'mobile safari', 'mobileexplorer',
            'opera mobi', 'opera mini', 'netfront', 'opwv', 'polaris', 'semc-browser', 'up.browser', 'webpro',
            'wms pie', 'xiino',
    ...
    */
    public function get_mobile_data($pv_browser_user_agent)
    {
        ...
        $a_mobile_browser = array('avantgo', 'blazer', 'elaine', 'eudoraweb', 'iemobile',  'minimo', 'mobile safari',
                                  'mobileexplorer', 'opera mobi', 'opera mini', 'netfront', 'opwv', 'polaris',
                                  'semc-browser', 'up.browser', 'webpro', 'wms pie', 'xiino');
        /*
        ...
        */
        $a_mobile_device = array('benq', 'blackberry', 'danger hiptop', 'ddipocket', ' droid', 'htc_dream',
                                 'htc espresso', 'htc hero', 'htc halo', 'htc huangshan', 'htc legend', 'htc liberty',
                                 'htc paradise', 'htc supersonic', 'htc tattoo', 'ipad', 'ipod', 'iphone', 'kindle',
                                 'lge-cx', 'lge-lx', 'lge-mx', 'lge vx', 'lg;lx', 'nintendo wii', 'nokia', 'palm',
                                 'pdxgw', 'playstation', 'sagem', 'samsung', 'sec-sgh', 'sharp', 'sonyericsson',
                                 'sprint', 'zunehd', 'zune', 'j-phone', 'milestone', 'n410', 'mot 24', 'mot-',
        ...
        */
        $a_mobile_os = array('android', 'epoc', 'cpu os', 'iphone os', 'palmos', 'palmsource', 'windows phone os',
    ...
    */
    ...
    /**
     * Update a ini file passing a array values, this public function don't remove the original comments
     * @author Erik Amaru Ortiz <erik@colosa.com>
