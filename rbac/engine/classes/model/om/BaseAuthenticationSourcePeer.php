...
require_once 'classes/model/map/AuthenticationSourceTableMap.php';
/**
...
abstract class BaseAuthenticationSourcePeer {
	...
	const OM_CLASS = 'AuthenticationSource';

	/** A class that can be returned by this peer. */
	const CLASS_DEFAULT = 'classes.model.AuthenticationSource';

	/** the related TableMap class for this table */
	const TM_CLASS = 'AuthenticationSourceTableMap';
	...
	/**
	 * An identiy map to hold any loaded instances of AuthenticationSource objects.
	 * This must be public so that other peer classes can access this when hydrating from JOIN
	 * queries.
	 * @var        array AuthenticationSource[]
	 */
	...
	protected static $fieldNames = array (
		...
		BasePeer::TYPE_STUDLYPHPNAME => array ('authSourceUid', 'authSourceName', 'authSourceProvider', 'authSourceServerName', 'authSourcePort', 'authSourceEnabledTls', 'authSourceVersion', 'authSourceBaseDn', 'authAnonymous', 'authSourceSearchUser', 'authSourcePassword', 'authSourceAttributes', 'authSourceObjectClasses', 'authSourceData', ),
		BasePeer::TYPE_COLNAME => array (self::AUTH_SOURCE_UID, self::AUTH_SOURCE_NAME, self::AUTH_SOURCE_PROVIDER, self::AUTH_SOURCE_SERVER_NAME, self::AUTH_SOURCE_PORT, self::AUTH_SOURCE_ENABLED_TLS, self::AUTH_SOURCE_VERSION, self::AUTH_SOURCE_BASE_DN, self::AUTH_ANONYMOUS, self::AUTH_SOURCE_SEARCH_USER, self::AUTH_SOURCE_PASSWORD, self::AUTH_SOURCE_ATTRIBUTES, self::AUTH_SOURCE_OBJECT_CLASSES, self::AUTH_SOURCE_DATA, ),
		BasePeer::TYPE_RAW_COLNAME => array ('AUTH_SOURCE_UID', 'AUTH_SOURCE_NAME', 'AUTH_SOURCE_PROVIDER', 'AUTH_SOURCE_SERVER_NAME', 'AUTH_SOURCE_PORT', 'AUTH_SOURCE_ENABLED_TLS', 'AUTH_SOURCE_VERSION', 'AUTH_SOURCE_BASE_DN', 'AUTH_ANONYMOUS', 'AUTH_SOURCE_SEARCH_USER', 'AUTH_SOURCE_PASSWORD', 'AUTH_SOURCE_ATTRIBUTES', 'AUTH_SOURCE_OBJECT_CLASSES', 'AUTH_SOURCE_DATA', ),
	...
	protected static $fieldKeys = array (
		...
		BasePeer::TYPE_STUDLYPHPNAME => array ('authSourceUid' => 0, 'authSourceName' => 1, 'authSourceProvider' => 2, 'authSourceServerName' => 3, 'authSourcePort' => 4, 'authSourceEnabledTls' => 5, 'authSourceVersion' => 6, 'authSourceBaseDn' => 7, 'authAnonymous' => 8, 'authSourceSearchUser' => 9, 'authSourcePassword' => 10, 'authSourceAttributes' => 11, 'authSourceObjectClasses' => 12, 'authSourceData' => 13, ),
		BasePeer::TYPE_COLNAME => array (self::AUTH_SOURCE_UID => 0, self::AUTH_SOURCE_NAME => 1, self::AUTH_SOURCE_PROVIDER => 2, self::AUTH_SOURCE_SERVER_NAME => 3, self::AUTH_SOURCE_PORT => 4, self::AUTH_SOURCE_ENABLED_TLS => 5, self::AUTH_SOURCE_VERSION => 6, self::AUTH_SOURCE_BASE_DN => 7, self::AUTH_ANONYMOUS => 8, self::AUTH_SOURCE_SEARCH_USER => 9, self::AUTH_SOURCE_PASSWORD => 10, self::AUTH_SOURCE_ATTRIBUTES => 11, self::AUTH_SOURCE_OBJECT_CLASSES => 12, self::AUTH_SOURCE_DATA => 13, ),
		BasePeer::TYPE_RAW_COLNAME => array ('AUTH_SOURCE_UID' => 0, 'AUTH_SOURCE_NAME' => 1, 'AUTH_SOURCE_PROVIDER' => 2, 'AUTH_SOURCE_SERVER_NAME' => 3, 'AUTH_SOURCE_PORT' => 4, 'AUTH_SOURCE_ENABLED_TLS' => 5, 'AUTH_SOURCE_VERSION' => 6, 'AUTH_SOURCE_BASE_DN' => 7, 'AUTH_ANONYMOUS' => 8, 'AUTH_SOURCE_SEARCH_USER' => 9, 'AUTH_SOURCE_PASSWORD' => 10, 'AUTH_SOURCE_ATTRIBUTES' => 11, 'AUTH_SOURCE_OBJECT_CLASSES' => 12, 'AUTH_SOURCE_DATA' => 13, ),
	...
	public static function addSelectColumns(Criteria $criteria, $alias = null)
	{
		if (null === $alias) {
			...
			$criteria->addSelectColumn($alias . '.AUTH_SOURCE_UID');
			$criteria->addSelectColumn($alias . '.AUTH_SOURCE_NAME');
			$criteria->addSelectColumn($alias . '.AUTH_SOURCE_PROVIDER');
			$criteria->addSelectColumn($alias . '.AUTH_SOURCE_SERVER_NAME');
			$criteria->addSelectColumn($alias . '.AUTH_SOURCE_PORT');
			$criteria->addSelectColumn($alias . '.AUTH_SOURCE_ENABLED_TLS');
			$criteria->addSelectColumn($alias . '.AUTH_SOURCE_VERSION');
			$criteria->addSelectColumn($alias . '.AUTH_SOURCE_BASE_DN');
			$criteria->addSelectColumn($alias . '.AUTH_ANONYMOUS');
			$criteria->addSelectColumn($alias . '.AUTH_SOURCE_SEARCH_USER');
			$criteria->addSelectColumn($alias . '.AUTH_SOURCE_PASSWORD');
			$criteria->addSelectColumn($alias . '.AUTH_SOURCE_ATTRIBUTES');
			$criteria->addSelectColumn($alias . '.AUTH_SOURCE_OBJECT_CLASSES');
			$criteria->addSelectColumn($alias . '.AUTH_SOURCE_DATA');
		}
	}
	...
	public static function doCount(Criteria $criteria, $distinct = false, PropelPDO $con = null)
	{
		...
		$criteria->setPrimaryTableName(AuthenticationSourcePeer::TABLE_NAME);
	...
	{
		return AuthenticationSourcePeer::populateObjects(AuthenticationSourcePeer::doSelectStmt($criteria, $con));
	}
	/**
	 ...
	 *
	 * @param      AuthenticationSource $value A AuthenticationSource object.
	...
	public static function addInstanceToPool($obj, $key = null)
	{
		if (Propel::isInstancePoolingEnabled()) {
			if ($key === null) {
				$key = (string) $obj->getAuthSourceUid();
			} // if key === null
	 ...
	 *
	 * @param      mixed $value A AuthenticationSource object or a primary key value.
	 */
	public static function removeInstanceFromPool($value)
	{
		if (Propel::isInstancePoolingEnabled() && $value !== null) {
			if (is_object($value) && $value instanceof AuthenticationSource) {
				$key = (string) $value->getAuthSourceUid();
			} elseif (is_scalar($value)) {
				// assume we've been passed a primary key
				$key = (string) $value;
			} else {
				$e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or AuthenticationSource object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value,true)));
				throw $e;
			}
	 ...
	 * @return     AuthenticationSource Found object or NULL if 1) no instance exists for specified key or 2) instance pooling has been disabled.
	...
	/**
	 * Method to invalidate the instance pool of all tables related to AUTHENTICATION_SOURCE
	...
	public static function populateObjects(PDOStatement $stmt)
	{
		...
		$cls = AuthenticationSourcePeer::getOMClass(false);
		// populate the object(s)
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$key = AuthenticationSourcePeer::getPrimaryKeyHashFromRow($row, 0);
			if (null !== ($obj = AuthenticationSourcePeer::getInstanceFromPool($key))) {
				...
				$obj->hydrate($row);
				$results[] = $obj;
				AuthenticationSourcePeer::addInstanceToPool($obj, $key);
			} // if key exists
		}
		$stmt->closeCursor();
		return $results;
	 ...
	 * @throws     PropelException Any exceptions caught during processing will be
	 *		 rethrown wrapped into a PropelException.
	 * @return     array (AuthenticationSource object, last column rank)
	 */
	public static function populateObject($row, $startcol = 0)
	{
		$key = AuthenticationSourcePeer::getPrimaryKeyHashFromRow($row, $startcol);
		if (null !== ($obj = AuthenticationSourcePeer::getInstanceFromPool($key))) {
			// We no longer rehydrate the object, since this can cause data loss.
			// See http://www.propelorm.org/ticket/509
			// $obj->hydrate($row, $startcol, true); // rehydrate
			$col = $startcol + AuthenticationSourcePeer::NUM_HYDRATE_COLUMNS;
		} else {
			$cls = AuthenticationSourcePeer::OM_CLASS;
			$obj = new $cls();
	...
	{
	  $dbMap = Propel::getDatabaseMap(BaseAuthenticationSourcePeer::DATABASE_NAME);
	  if (!$dbMap->hasTable(BaseAuthenticationSourcePeer::TABLE_NAME))
	  {
	    $dbMap->addTableObject(new AuthenticationSourceTableMap());
	  }
	}
	...
	{
		return $withPrefix ? AuthenticationSourcePeer::CLASS_DEFAULT : AuthenticationSourcePeer::OM_CLASS;
	}

	/**
	 * Performs an INSERT on the database, given a AuthenticationSource or Criteria object.
	 *
	...
	/**
	 * Performs an UPDATE on the database, given a AuthenticationSource or Criteria object.
	 *
	...
	public static function doUpdate($values, PropelPDO $con = null)
	{
		...
		if ($values instanceof Criteria) {
			...
			$value = $criteria->remove(AuthenticationSourcePeer::AUTH_SOURCE_UID);
			if ($value) {
				$selectCriteria->add(AuthenticationSourcePeer::AUTH_SOURCE_UID, $value, $comparison);
			} else {
				$selectCriteria->setPrimaryTableName(AuthenticationSourcePeer::TABLE_NAME);
			}
	...
	/**
	 * Deletes all rows from the AUTHENTICATION_SOURCE table.
	 *
	 * @param      PropelPDO $con the connection to use
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
	public static function doDeleteAll(PropelPDO $con = null)
	{
		...
		try {
			...
			$con->beginTransaction();
			$affectedRows += BasePeer::doDeleteAll(AuthenticationSourcePeer::TABLE_NAME, $con, AuthenticationSourcePeer::DATABASE_NAME);
			// Because this db requires some delete cascade/set null emulation, we have to
			// clear the cached instance *after* the emulation has happened (since
			// instances get re-added by the select statement contained therein).
			AuthenticationSourcePeer::clearInstancePool();
			AuthenticationSourcePeer::clearRelatedInstancePool();
			$con->commit();
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
	...
	/**
	 * Performs a DELETE on the database, given a AuthenticationSource or Criteria object OR a primary key value.
	 *
	 ...
	 */
	 public static function doDelete($values, PropelPDO $con = null)
	 {
		...
		if ($values instanceof Criteria) {
		...
		} elseif ($values instanceof AuthenticationSource) { // it's a model object
			// invalidate the cache for this single object
			AuthenticationSourcePeer::removeInstanceFromPool($values);
			...
			foreach ((array) $values as $singleval) {
				AuthenticationSourcePeer::removeInstanceFromPool($singleval);
			}
		}
	...
	public static function retrieveByPK($pk, PropelPDO $con = null)
	{
		...
		if (null !== ($obj = AuthenticationSourcePeer::getInstanceFromPool((string) $pk))) {
			return $obj;
		}
...
//
BaseAuthenticationSourcePeer::buildTableMap();
